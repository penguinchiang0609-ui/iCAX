...
{
    iCAX::Core::LogLevel _nLogLevel = (iCAX::Core::LogLevel)m_pSettingService->GetApplicationSetting().Get("Logger", "LogLevel", iCAX::Data::Variant(0)).To<int>();
    ...
    const size_t _nRetainFileCount = static_cast<size_t>((std::max)(_nMaxFiles, 0));
    ...
    try
    {
        ...
        if (logs.size() > _nRetainFileCount)
        {
            ...
            for (size_t i = 0; i < logs.size() - _nRetainFileCount; ++i)
            {
