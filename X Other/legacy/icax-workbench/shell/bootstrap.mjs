...
function createActions({ bridge, mailboxClient, store }) {
  ...
  function productClientFor(product) {
    const productChannelId = product?.productChannelId;
    if (!productChannelId) {
      throw new Error("Product is not started");
    }

    if (!productClients.has(productChannelId)) {
      productClients.set(productChannelId, new ProductClient(mailboxClient, productChannelId));
    }
    return productClients.get(productChannelId);
  }

  return {
    async bootstrap() {
      store.setBridgeStatus("Connecting");
      const applicationChannelId = await bridge.getApplicationChannelId();
      appClient = new ApplicationClient(mailboxClient, applicationChannelId);
      store.patch((state) => {
        state.applicationChannelId = applicationChannelId;
      });
