...
function createProductClient(mailboxClient, product) {
  if (!mailboxClient || !product?.productChannelId) {
    return null;
  }
  return new ProductClient(mailboxClient, product.productChannelId);
}

function createProjectClient(mailboxClient, project) {
  if (!mailboxClient || !project?.projectChannelId) {
    return null;
  }
  return new ProjectClient(mailboxClient, project.projectChannelId);
}
