...
namespace
{
    ...
    void ReleaseMailPayload(IN OUT iCAX::Mail::Mail& Mail_) noexcept
    {
...
TEST(ProductRuntimeMailboxTest, ProductMailboxCanOpenAndListProjectCatalogs)
{
    ...
    EXPECT_FALSE(_MainProject.at("projectMailId").To<iCAX::Data::uuid>().is_nil());
...
TEST(ProductRuntimeMailboxTest, ProductCommandSentToProjectMailboxIsRejected)
{
    ...
    EXPECT_EQ(iCAX::Mail::kMailInvalidPayload, _Responses[0].Header.nStamp);
    EXPECT_EQ(3001u, _Responses[0].Header.nOriginId);

    ReleaseMailPayloads(_Responses);
    _pRuntime->Stop();
}

TEST(ProductRuntimeMailboxTest, ProjectMailboxProvidesProjectRuntimeContext)
{
    constexpr uint64_t kInspectProjectRuntimeContextCommand = 0x7000000000001001ull;

    auto _pRuntime = MakeRuntime();
    _pRuntime->Start();
    auto _pCatalog = _pRuntime->OpenProjectCatalog("Runtime Context", {}, "Runtime Context");
    ASSERT_NE(nullptr, _pCatalog);
    auto _pProject = _pCatalog->GetMainProject();
    ASSERT_NE(nullptr, _pProject);

    const auto _ProjectID = _pProject->GetProjectID();
    _pRuntime->GetCommandRegistry().Set(
        kInspectProjectRuntimeContextCommand,
        std::make_shared<iCAX::Command::CFunctionCommandHandler>(
            [_ProjectID](
                ...
                auto _pProjectRuntime = Context_.RequireDependency<iCAX::Project::IProjectRuntime>();
                auto _pLocalProject = Context_.RequireDependency<iCAX::Project::CProject>();

                iCAX::Data::ObjectMap _Payload;
                _Payload["projectId"] = _pProjectRuntime->GetProjectID();
                _Payload["projectMailId"] = _pProjectRuntime->GetProjectMailID();
                _Payload["projectName"] = _pProjectRuntime->GetProjectName();
                _Payload["hasLocalProject"] = _pLocalProject != nullptr;
                _Payload["sameProject"] = _pLocalProject
                    && _pLocalProject->GetProjectID() == _ProjectID
                    && _pProjectRuntime->GetProjectID() == _ProjectID;
    ...
    EXPECT_FALSE(_Payload.at("projectMailId").To<iCAX::Data::uuid>().is_nil());
    EXPECT_EQ("Runtime Context", _Payload.at("projectName").To<std::string>());
    EXPECT_TRUE(_Payload.at("hasLocalProject").To<bool>());
