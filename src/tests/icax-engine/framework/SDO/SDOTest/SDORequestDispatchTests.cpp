...
TEST(SDOFrameDispatchTest, InvokesSDOAndRespondsWithTheRequestCallID)
{
    auto _pRegistry = std::make_shared<CSDORegistry>();
    auto _pSDO = std::make_shared<CTestSDO>("Echo");
    ASSERT_TRUE(_pSDO->ExposeMethod("Ping", [](
    ...
    ASSERT_TRUE(_pRegistry->Register(_pSDO));

    CSDOInvoker _Invoker(_pRegistry);
    ...
    _Frontend.Send(CreateTextSDOFrame(
        21,
        MakeSDOMethodCode("Echo", "Ping"),
        ESDOFrameKind::Request,
        "pong"));
    ...
    EXPECT_EQ(ESDOFrameKind::Response, _Frames[0].nKind);
    EXPECT_EQ(EInvocationStatus::Ok, _Frames[0].nStatus);
    EXPECT_EQ("pong", GetSDOPayloadText(_Frames[0]));
}

TEST(SDOFrameDispatchTest, ReportsAndResponseShareOneCallID)
{
    auto _pRegistry = std::make_shared<CSDORegistry>();
    auto _pSDO = std::make_shared<CTestSDO>("Echo");
    ASSERT_TRUE(_pSDO->ExposeMethod("LongTask", [](
        IN const CInvocation& Call_,
        IN const iCAX::Application::IApplicationContext&,
        IN iCAX::Product::IProductContext*,
        IN iCAX::Project::IProjectContext*,
    ...
    _Frontend.Send(CreateTextSDOFrame(
        41,
        MakeSDOMethodCode("Echo", "LongTask"),
        ESDOFrameKind::Request,
        "{}"));
    EXPECT_EQ(1u, _Invoker.DispatchAvailableFrames(_Backend, _Context, nullptr, nullptr, nullptr));
    ...
    EXPECT_EQ(ESDOFrameKind::Report, _Frames[0].nKind);
    EXPECT_EQ(ESDOFrameKind::Report, _Frames[1].nKind);
    EXPECT_EQ(ESDOFrameKind::Response, _Frames[2].nKind);
    EXPECT_TRUE(std::all_of(_Frames.begin(), _Frames.end(), [](const CSDOFrame& Frame_) {
        return Frame_.nCallID == 41;
    }));
    EXPECT_EQ("25%", GetSDOPayloadText(_Frames[0]));
    EXPECT_EQ("75%", GetSDOPayloadText(_Frames[1]));
    EXPECT_EQ("done", GetSDOPayloadText(_Frames[2]));
}

TEST(SDOFrameDispatchTest, ReturnsTypedNotFoundStatus)
{
    auto _pRegistry = std::make_shared<CSDORegistry>();
    CSDOInvoker _Invoker(_pRegistry);
    CTestApplicationContext _Context;
    ...
    _Frontend.Send(CreateTextSDOFrame(
        31,
        MakeSDOMethodCode("Missing", "Ping"),
        ESDOFrameKind::Request,
        "{}"));
    EXPECT_EQ(1u, _Invoker.DispatchAvailableFrames(_Backend, _Context, nullptr, nullptr, nullptr));
    ...
    EXPECT_EQ(EInvocationStatus::SDONotFound, _Frames[0].nStatus);
    EXPECT_NE(std::string::npos, GetSDOPayloadText(_Frames[0]).find("SDO not found"));
}

TEST(SDORemoteInvocationTest, BackendCanCallFrontendSDOWithTheSameCallID)
{
    auto _pRegistry = std::make_shared<CSDORegistry>();
    CSDOInvoker _BackendInvoker(_pRegistry);
    CTestApplicationContext _Context;
    CSDOChannel _Channel;
    auto _Frontend = _Channel.GetEndAEndpoint();
    auto _Backend = _Channel.GetEndBEndpoint();
    ...
    const auto _Method = MakeSDOMethod("Frontend", "GetSelection");
    ...
    EXPECT_EQ(ESDOFrameKind::Request, _Requests[0].nKind);

    _Frontend.Send(CreateTextSDOFrame(
        _CallID,
        _Method.GetCode(),
        ESDOFrameKind::Report,
        "reading"));
    _Frontend.Send(CreateTextSDOFrame(
        _CallID,
        _Method.GetCode(),
        ESDOFrameKind::Response,
        "part-42"));
...
TEST(SDORemoteInvocationTest, CallRemoteTaskUsesProvidedScheduler)
{
    auto _pRegistry = std::make_shared<CSDORegistry>();
    CSDOInvoker _BackendInvoker(_pRegistry);
    CTestApplicationContext _Context;
    ...
    const auto _Method = MakeSDOMethod("Frontend", "GetState");
    auto _Task = _BackendInvoker.CallRemote(
        _Backend,
        _Method,
        {},
