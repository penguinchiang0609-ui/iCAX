...
`framework` 测试目录与 `src/icax-engine/framework` 一一对应，放置 framework 下各项目的单元测试。

## 目录结构

- `ApplicationContext/`：对应 `src/icax-engine/framework/ApplicationContext` 的 ApplicationContext 单元测试。
- `SDO/`：对应 `src/icax-engine/framework/SDO` 的 SDO 单元测试，包括请求分发、汇报和结果返回。
- `Database/`：对应 `src/icax-engine/framework/Database` 的 Database 单元测试。
- `SDO/`：对应 `src/icax-engine/framework/SDO` 的 SDO 调用单元测试。
