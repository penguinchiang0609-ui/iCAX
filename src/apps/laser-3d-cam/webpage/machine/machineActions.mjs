...
export async function handleMachineRibbonCommand(context, view, commandId, ops) {
  ...
  if (commandId === "machine.home") {
    runMachineCommand(context, view, "Machine.Home", {}, {}, ops);
    return true;
  }
  if (commandId === "machine.reset") {
    runMachineCommand(context, view, "Machine.Reset", {}, {}, ops);
    return true;
  }
  if (commandId === "machine.limit-check") {
    runMachineCommand(context, view, "Machine.CheckLimits", {}, {}, ops);
    return true;
  }
  if (commandId === "machine.reach-check") {
    runMachineCommand(context, view, "Machine.CheckReach", {}, {}, ops);
    return true;
  }
  return false;
}
...
async function applyMachineAppearanceLive(context, view, editor, ops) {
  ...
  try {
    const responsePayload = await context.sceneProxy.execute("Machine.SetElementAppearance", payload, { timeoutMs: 10000 });
    if (sequence !== state.sequence) {
      return;
    }
    mergeMachineCommandPayload(view, responsePayload);
  } catch (error) {
    if (sequence !== state.sequence) {
      return;
    }
...
export async function chooseMachineFile(context, view, importAfterChoose, ops) {
  ...
  try {
    const path = await bridge.openFileDialog({
      title: "选择机床定义文件",
      filters: [
        { name: "SDF Machine Definition", extensions: ["sdf", "xml"] },
      ],
    });
    if (!path) {
      return;
    }
    ops.appendProjectLog(context, "info", `选择 SDF 机床定义文件：${path}`);
...
export async function importMachinePath(context, view, sourcePath, ops) {
  ...
  view.machineSourcePath = path;
  if (!path) {
    view.error = "请输入 SDF/XML 机床定义文件路径";
    ops.renderProject(context, view);
    return;
  }
...
export async function instantiateMachineDefinition(context, view, machineDefinitionId, ops) {
  ...
  const ok = await ops.runProjectCommand(context, view, "Machine.Instantiate", { machineDefinitionId: id }, { timeoutMs: 60000 });
  if (ok) {
...
export async function setMachineDefinitionEnabled(context, view, machineDefinitionId, enabled, ops) {
  ...
  view.selectedMachineDefinitionId = id;
  await ops.runProjectCommand(context, view, "MachineDefinition.SetEnabled", { machineDefinitionId: id, enabled: Boolean(enabled) });
}
...
export async function deleteMachineDefinition(context, view, machineDefinitionId, ops) {
  ...
  const ok = await ops.runProjectCommand(context, view, "MachineDefinition.Delete", { machineDefinitionId: id });
...
export async function setMachineInstanceEnabled(context, view, machineEntityId, enabled, ops) {
  ...
  view.selectedMachineInstanceId = id;
  const result = await ops.runProjectCommand(context, view, "Machine.SetEnabled", { machineEntityId: id, enabled: Boolean(enabled) });
  if (result) {
    await ops.fitViewAfterRenderPublish(context, view);
  }
}
...
export function saveMachineParameters(context, view, ops) {
  ...
  runMachineCommand(context, view, "Machine.SetParameters", {
    maxVelocity,
    maxAcceleration,
    linearJogStep,
    angularJogStep,
...
export function saveMachineTCP(context, view, ops) {
  runMachineCommand(context, view, "Machine.SetTCP", {
    tcp: [
...
async function applyMachineElementTransformLive(context, view, editor, ops) {
  ...
  try {
    const responsePayload = await context.sceneProxy.execute("Machine.SetElementTransform", payload, { timeoutMs: 10000 });
    if (sequence !== state.sequence) {
      return;
    }
...
async function applyMachineJointLimitsLive(context, view, editor, ops) {
  ...
  try {
    const responsePayload = await context.sceneProxy.execute("Machine.SetJointLimits", payload, { timeoutMs: 10000 });
    if (sequence !== state.sequence) {
      return;
    }
    mergeMachineCommandPayload(view, responsePayload);
...
async function applyMachineToolTCPLive(context, view, editor, ops) {
  ...
  try {
    const responsePayload = await context.sceneProxy.execute("Machine.SetToolTCP", payload, { timeoutMs: 10000 });
    if (sequence !== state.sequence) {
      return;
    }
    mergeMachineCommandPayload(view, responsePayload);
...
function mergeMachineCommandPayload(view, payload) {
...
export function jogMachine(context, view, direction, ops) {
  ...
  runMachineCommand(context, view, "Machine.Jog", { axis, delta }, {}, ops);
}

export function runMachineCommand(context, view, command, payload = {}, options = {}, ops) {
  ...
  ops.runProjectCommand(context, view, command, { machineEntityId, ...payload }, options);
}
