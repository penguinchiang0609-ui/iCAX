...
export function handleMachiningRibbonCommand(context, view, commandId, ops) {
  if (commandId === "toolpath.recognize-holes") {
    ops.invokeSDOMethod(context, view, "Toolpath.RecognizeLoops", {});
    return true;
  }
  if (commandId === "toolpath.add-selection") {
    ops.invokeSDOMethod(context, view, "Toolpath.AddSelectionPath", {});
    return true;
  }
  if (commandId === "toolpath.clear") {
    ops.invokeSDOMethod(context, view, "Toolpath.ClearProgram", {});
    return true;
  }
  if (commandId === "toolpath.clear-selection") {
    ops.invokeSDOMethod(context, view, "Selection.PickTopology", { kind: "", id: 0, label: "" });
    return true;
  }
...
export async function setJobMachine(context, view, machineEntityId, ops) {
  ...
  await ops.invokeSDOMethod(context, view, "Job.SetMachine", { machineEntityId: id });
}
