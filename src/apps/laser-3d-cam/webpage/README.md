...
- `mountProject`：绑定当前 ProjectProxy 和 SceneProxy，调用 `Cam.Scene.Get` 获取项目场景状态。
...
- 模型导入调用 `Cam.Workpiece.Import`，当前只接受 STEP/STP、IGS/IGES 工件路径；CEF 宿主提供 `openFileDialog` 时页面可以直接选择模型文件。
