...
| 引用 | 来源 | 说明 |
| --- | --- | --- |
| `system:post-cap` | 产品安装目录 `templates/accessory/post-cap/` | 随产品提供的固定尺寸模型 |
...
目录为 `templates/accessory/<id>/model.json` 和描述符指定的实体文件。例如：
...
`path` 必须是模板包内相对路径，使用 `/`；不允许绝对路径、盘符、`..` 或通过链接跳出包目录。未声明的 `template:` 引用必须失败，不能临时猜测文件位置。示例文件位于对应产品包的 `resource/` 目录。
