...
def generate(p, context):
    ...
    if p["bridge"] >= 2 * h:
        raise ValueError("底部保留厚度须小于该方向截面高度")
    bottom, top = -h + p["bridge"], h + 1; depth = top - bottom
    left, right = math.radians(p["leftAngle"]), math.radians(p["rightAngle"])
    if p["rootRadius"] and p["rootWidth"]:
        raise ValueError("槽底圆角和平口宽度不能同时使用")
    if p["rootRadius"]:
        radius = p["rootRadius"]
        ly, ry = bottom + radius * (1 - math.sin(left)), bottom + radius * (1 - math.sin(right))
        lx, rx = -radius * math.cos(left), radius * math.cos(right)
        if max(ly, ry) >= top:
            raise ValueError("槽底圆弧超过可切除高度，请减小圆弧半径")
        lt, rt = [lx - (top - ly) * math.tan(left), top], [rx + (top - ry) * math.tan(right), top]
        contour = {"kind": "path", "segments": [
            {"kind": "line", "start": lt, "end": [lx, ly]}, {"kind": "arc", "start": [lx, ly], "middle": [0, bottom], "end": [rx, ry]},
            {"kind": "line", "start": [rx, ry], "end": rt}, {"kind": "line", "start": rt, "end": lt}]}
    else:
        width = p["rootWidth"] / 2
        points = [[-width - depth * math.tan(left), top], [-width, bottom]]
        if width: points.append([width, bottom])
        points.append([width + depth * math.tan(right), top])
    ...
    if p["reliefDiameter"]:
        ...
        prism("relief", circle, bottom + p["reliefLift"]); cutters.append("relief")
    if p["bottomCut"]:
        half = p["bottomCutWidth"] / 2; points = [[-half, -h - 1], [half, -h - 1], [half, bottom + 1], [-half, bottom + 1]]
        prism("bottom-cut", {"kind": "path", "segments": [{"kind": "line", "start": a, "end": b} for a, b in zip(points, points[1:] + points[:1])]}); cutters.append("bottom-cut")
    output = "notch"
    if len(cutters) > 1:
        nodes.append({"key": "tool", "operator": "boolean", "inputs": cutters, "arguments": {"operation": "union"}}); output = "tool"
    return {"mode": "solid", "coordinateSpace": "part", "outputKey": output, "model": {"schema": "icax.neutral-model", "schemaVersion": 1, "template": {"id": "v-notch-asymmetric", "version": "1.0.0", "packageDigest": "self-contained"}, "geometry": nodes}}
