...
def generate(p, context):
    ...
    center = anchor + sign * radius/sa
    if radius < abs(sr)*(hi[1]-lo[1])/2 + abs(cr)*(hi[2]-lo[2])/2 + abs(p["offset"]):
        raise ValueError("凸头圆柱须覆盖整个截面宽度，请增大直径或减小偏移")
    origin = [center-axis[0]*reach,yc-p["offset"]*sr-axis[1]*reach,zc+p["offset"]*cr-axis[2]*reach]
    prism("cylinder", origin, tangent, cross, {"kind":"circle","radius":radius}, [a*2*reach for a in axis])
    slab("end-slab", center)
    boolean("tool", "subtract", "end-slab", "cylinder")
    datum = None
