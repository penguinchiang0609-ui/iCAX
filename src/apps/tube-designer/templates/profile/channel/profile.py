...
def strip(points,thickness,inner_radius,outer_radius=None,inner_offset=(0,0),free_end_radii=(0,0)):
    """Bent strip with an independently positioned inner boundary.

    ``points`` defines the fixed outer boundary reference.  ``inner_offset``
    moves only the inner boundary, just like the eccentric inner contour of a
    rectangular tube; it must never translate the whole section.
    """
    positive(thickness)
    if (not isinstance(free_end_radii,(list,tuple)) or len(free_end_radii)!=2
            or any(not math.isfinite(value) or value<0 for value in free_end_radii)):
        raise ValueError("自由端圆角必须为两个非负数")
    if (not isinstance(inner_offset,(list,tuple)) or len(inner_offset)!=2
            or any(not math.isfinite(v) for v in inner_offset)):
        raise ValueError("内轮廓偏移必须为有限二维坐标")
    def value_at(value,index,kind):
        if isinstance(value,(list,tuple)):
            if len(value)!=len(points)-2:raise ValueError(f"{kind}数量必须与折弯数量一致")
            value=value[index]
        if not math.isfinite(value) or value<0:raise ValueError(f"{kind}不能为负数")
        return value
    directions=[]
    for a,b in zip(points,points[1:]):
        u=[b[0]-a[0],b[1]-a[1]];length=math.hypot(*u);positive(length)
        directions.append([x/length for x in u])
    def make_side(source_points,sign,round_free_ends=False):
        vertices=[];rs=[]
        for i,p in enumerate(source_points):
            a=directions[max(0,i-1)];b=directions[min(i,len(directions)-1)]
            if i in (0,len(points)-1):
                vertices.append([p[0]-sign*b[1]*thickness/2,p[1]+sign*b[0]*thickness/2])
                rs.append(free_end_radii[0 if i==0 else 1] if round_free_ends else 0);continue
            ...
            bend_index=i-1
            inner=value_at(inner_radius,bend_index,"内弯半径")
            outer=(inner+thickness if outer_radius is None else
                   value_at(outer_radius,bend_index,"外弯半径"))
            rs.append(inner if sign*cross>0 else outer)
        return vertices,rs

    outer_side,outer_corner_radii=make_side(points,1)
    # Keep each open-end cap normal to its flange.  The inner bend/web
    # geometry follows the eccentricity, while the two free-end x positions
    # stay aligned with the fixed outer boundary so the caps cannot slant.
    shifted_inner=[[
        p[0] if i in (0,len(points)-1) else p[0]+inner_offset[0],
        p[1]+inner_offset[1],
    ] for i,p in enumerate(points)]
    # One free-end radius describes one toe. Apply it only to the
    # inner/toe transition, rather than creating two unrelated arcs on the
    # two sides of the end cap.
    inner_side,inner_corner_radii=make_side(shifted_inner,-1,True)
    sides=[outer_side,inner_side]
    corner_radii=[outer_corner_radii,inner_corner_radii]
...
PARAMETERS = ("width", "depth", "wallThickness", "outerRadius", "useHotRolled", "flangeThickness", "useOuterRadii", "outerRadius1", "outerRadius2", "useInnerRadius", "innerRadius1", "innerRadius2", "freeEndRadius", "useIndependentFreeEndRadii", "freeEndRadius1", "freeEndRadius2", "innerOffsetX", "innerOffsetY")


HOT_SLOPE = 0.1


def _hot_rolled_strip(width, depth, wall, flange, outer_radii, inner_radii,
                      inner_offset=(0, 0), free_end_radii=(0, 0)):
    """Nominal GB/T 706-style channel: inner legs slope 1:10.

    ``flange`` is the thickness at the midpoint of each leg.  The outside
    envelope stays on the requested width/depth; only the inner contour is
    shifted by the existing eccentricity parameters.
    """
    positive(width, depth, wall, flange)
    if not isinstance(inner_offset, (list, tuple)) or len(inner_offset) != 2:
        raise ValueError("内轮廓偏移必须为二维坐标")
    if any(not math.isfinite(v) for v in inner_offset):
        raise ValueError("内轮廓偏移必须为有限数值")
    if not isinstance(free_end_radii, (list, tuple)) or len(free_end_radii) != 2:
        raise ValueError("自由端圆角必须为两个非负数")
    if any(not math.isfinite(v) or v < 0 for v in free_end_radii):
        raise ValueError("自由端圆角必须为两个非负数")
    outer_bottom, outer_top = -depth / 2, depth / 2
    outer_web = -width / 2
    inner_web = outer_web + wall + inner_offset[0]
    inner_toe = width / 2
    span = inner_toe - inner_web
    if span <= 1e-6:
        raise ValueError("内轮廓偏移使槽腿长度无效")
    toe_thickness = flange - HOT_SLOPE * span / 2
    if toe_thickness <= 1e-6:
        raise ValueError("热轧槽钢槽口理论厚度必须大于零")
    bottom_mid = outer_bottom + flange + inner_offset[1]
    top_mid = outer_top - flange + inner_offset[1]
    bottom_root = bottom_mid + HOT_SLOPE * span / 2
    bottom_toe = bottom_mid - HOT_SLOPE * span / 2
    top_root = top_mid - HOT_SLOPE * span / 2
    top_toe = top_mid + HOT_SLOPE * span / 2
    points = [
        [inner_toe, outer_bottom],
        [outer_web, outer_bottom],
        [outer_web, outer_top],
        [inner_toe, outer_top],
        [inner_toe, top_toe],
        [inner_web, top_root],
        [inner_web, bottom_root],
        [inner_toe, bottom_toe],
    ]
    corner_radii = [
        0, outer_radii[0], outer_radii[1], 0,
        free_end_radii[1], inner_radii[1], inner_radii[0], free_end_radii[0],
    ]
    return polygon(points, corner_radii)


def build(p):
    p=dict(p)
    w,h=p["width"],p["depth"]
    dx,dy=p.get("innerOffsetX",0),p.get("innerOffsetY",0)
    outer_radii=([p["outerRadius1"],p["outerRadius2"]]
                 if p.get("useOuterRadii",False)
                 else [p["outerRadius"],p["outerRadius"]])
    inner_radii=([p["innerRadius1"],p["innerRadius2"]] if p.get("useInnerRadius",False)
                 else [max(radius-p["wallThickness"],0) for radius in outer_radii])
    free_end_radii=([p["freeEndRadius1"],p["freeEndRadius2"]]
                    if p.get("useHotRolled",False) and p.get("useIndependentFreeEndRadii",False)
                    else [p["freeEndRadius"],p["freeEndRadius"]]
                    if p.get("useHotRolled",False) else [0,0])
    if p.get("useHotRolled",False):
        contour=_hot_rolled_strip(w,h,p["wallThickness"],p["flangeThickness"],
                                  outer_radii,inner_radii,(dx,dy),free_end_radii)
    else:
        contour=strip([[w/2,-h/2],[-w/2,-h/2],
                       [-w/2,h/2],[w/2,h/2]],
                      p["wallThickness"],inner_radii,outer_radii,(dx,dy),free_end_radii)
    contours=[contour]
    width,depth=envelope(contours)
    positive(width,depth)
    return {"contours":contours,**p,"_parameters":p,"kind":"channel",
            "width":width,"depth":depth,
            "wallThickness":p.get("wallThickness",0),
            "cornerRadius":0,
            "specification":f"槽钢 {width:g} × {depth:g} mm",
            "manufacturingRoute":"HotRolledChannel" if p.get("useHotRolled",False) else "Parametric"}
