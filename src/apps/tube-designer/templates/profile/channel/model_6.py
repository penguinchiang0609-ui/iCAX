"""热轧平行翼缘槽钢: 参数化几何族；数值为演示尺寸，不宣称对应某一国标牌号/系列。"""
import math
...
def build(parameters):
    ...
    return {"contours": contours, **p, "_parameters": p, "kind": "channel-hot-parallel", "width": width, "depth": depth,
            "wallThickness": p.get("wallThickness", 0), "cornerRadius": p.get("cornerRadius", 0),
            "specification": f"热轧平行翼缘槽钢 {width:g} × {depth:g} mm"}
