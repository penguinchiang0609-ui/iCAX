"""外卷边 C 型钢: 模板负责本截面的参数和构造；公共层不按管型名称分支。"""
import math
...
def build(parameters):
    ...
    return {"contours": contours, **p, "_parameters": p, "kind": "channel-lipped-outward", "width": width, "depth": depth,
            "wallThickness": p.get("wallThickness", 0), "cornerRadius": p.get("cornerRadius", 0),
            "specification": f"外卷边 C 型钢 {width:g} × {depth:g} mm"}
