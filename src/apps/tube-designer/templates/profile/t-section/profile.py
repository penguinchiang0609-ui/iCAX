...
def build(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    if width <= 0 or depth <= 0 or thickness <= 0 or thickness >= min(width, depth):
        raise ValueError("T 型钢截面尺寸或厚度无效")
    return {
        "kind": "t-section",
        "width": width,
        "depth": depth,
        ...
        "cornerRadius": 0.0,
        "specification": f"T{_text(width)} × {_text(depth)} × {_text(thickness)}",
    }
...
def contours(
    ...
    if width <= 0 or depth <= 0 or thickness <= 0 or thickness >= min(width, depth):
        raise ValueError("T 型钢偏置后的二维轮廓无效")
...
def fitter(contours: list[dict[str, Any]], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return fitter_for_profile(contours, "t-section", context)
