...
def ellipse_path(center, width, height):
    positive(width, height)
    cx, cy = center
    if width >= height:
        major, minor, rotation = width / 2, height / 2, 0.0
    else:
        major, minor, rotation = height / 2, width / 2, math.pi / 2
    return {"kind": "path", "closed": True, "segments": [
        {"kind": "ellipseArc", "center": [cx, cy],
         "majorRadius": major, "minorRadius": minor,
         "rotation": rotation,
         "startAngle": math.tau * i / 4,
         "endAngle": math.tau * (i + 1) / 4}
        for i in range(4)
    ]}


def build_ellipse(parameters):
    p = dict(parameters)
    width, depth, thickness = p["width"], p["depth"], p["wallThickness"]
    inner_width = p["innerWidth"] or width - 2 * thickness
    inner_depth = p["innerDepth"] or depth - 2 * thickness
    offset_x, offset_y = p["innerOffsetX"], p["innerOffsetY"]
    positive(inner_width, inner_depth, width - inner_width, depth - inner_depth, thickness)
    if abs(offset_x) + inner_width / 2 >= width / 2 or abs(offset_y) + inner_depth / 2 >= depth / 2:
        raise ValueError("内孔偏心导致内外椭圆相交")
    contours = [
        ellipse_path([0, 0], width, depth),
        ellipse_path([offset_x, offset_y], inner_width, inner_depth),
    ]
    actual_width, actual_depth = envelope(contours)
    positive(actual_width, actual_depth)
    return {"contours": contours, **p, "_parameters": p, "kind": "ellipse",
            "width": actual_width, "depth": actual_depth,
            "wallThickness": thickness, "cornerRadius": 0,
            "specification": f"椭圆管 {actual_width:g} × {actual_depth:g} mm"}
...
def build(p):
    ...
    if actual:
        ...
        result=build_ellipse({
            "width": p["width"],
            "depth": p["depth"],
            "wallThickness": p["wallThickness"],
            "innerWidth": p["model0InnerWidth"],
            "innerDepth": p["model0InnerDepth"],
            "innerOffsetX": p["innerOffsetX"],
            "innerOffsetY": p["innerOffsetY"],
        })
    result["manufacturingRoute"] = "Unspecified"
    result.update({"kind":"oval","_familyParameters":dict(p),"specification":"椭圆管"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision","")})
    return result
