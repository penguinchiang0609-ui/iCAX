...
import importlib
import json
MODELS = {"round-bar":"round_bar"}
PARAMETERS = {"round-bar":{"width":"width"}}

def selected(p):
    name=p.get("sectionModel","round-bar")
...
def build(p):
    ...
    result["manufacturingRoute"] = {"round-bar":"Unspecified"}[name]
    result.update({"kind":"round-bar","_familyParameters":dict(p),"specification":"圆棒"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision",""),"sectionModel":name})
    return result
