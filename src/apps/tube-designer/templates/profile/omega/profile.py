...
MODELS = {"omega-cold":"model_0"}
PARAMETERS = {"omega-cold":{"width":"width","depth":"depth","wallThickness":"wallThickness","bendRadius":"bendRadius","crownWidth":"crownWidth","flangeWidth":"flangeWidth"}}

def selected(p):
    name=p.get("sectionModel","omega-cold")
...
def build(p):
    ...
    result["manufacturingRoute"] = {"omega-cold":"ColdBent"}[name]
    result.update({"kind":"omega","_familyParameters":dict(p),"specification":"帽型 / Omega 型钢"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision",""),"sectionModel":name})
    return result
