...
def _cold_contour(p):
    """冷弯帽钢：等厚理论中心折线 + 圆角 + 两侧偏置。"""
    b = float(p["width"])
    h = float(p["depth"])
    f = float(p["flangeWidth"])
    t = float(p["wallThickness"])
    r = float(p["bendRadius"])
    positive(b, h, f, t)
    _nonnegative(r)
    # B 是帽顶中心线宽，F 是单侧平腿中心线长度。
    centreline = [
        [-b / 2 - f, -h / 2],
        [-b / 2, -h / 2],
        [-b / 2, h / 2],
    ...
    return [strip(centreline, t, r)]


def _hot_contour(p):
    """热轧帽钢：分区厚度和四组轧制圆角直接构造，不走等厚偏置。"""
    h = float(p["hotHeight"])
    b1 = float(p["innerWidth"])
    b2 = float(p["hotFlangeWidth"])
    t1 = float(p["topThickness"])
    t2 = float(p["legThickness"])
    t3 = float(p["flangeThickness"])
    r1 = float(p["rootRadius1"])
    r2 = float(p["rootRadius2"])
    r3 = float(p["freeEndRadiusUpper"])
    r4 = float(p["freeEndRadiusLower"])
    positive(h, b1, b2, t1, t2, t3)
    _nonnegative(r1, r2, r3, r4)

    half_inner = b1 / 2
    outer_leg = half_inner + t2
    free_end = outer_leg + b2
    top = h / 2
    bottom = -h / 2
    inner_top = top - t1
    inner_bottom = bottom + t3
    if inner_top <= inner_bottom:
        raise ValueError("热轧帽钢总高度必须大于顶部和底部厚度之和")

    # r1/r2 是文档定义的内侧过渡半径。对应外侧圆角不能继续使用
    # 同一个半径，否则在厚度不为零时内外圆弧会相交。外侧半径按
    # 相邻两区的较大厚度扩展；等厚时退化为标准的 R+t 同心过渡。
    outer_r1 = r1 + max(t1, t2)
    outer_r2 = r2 + max(t2, t3)

    # 顺时针沿材料边界绕行。直腿外侧接平腿上表面，直腿内侧接
    # 平腿下表面；这与冷弯 strip 的边界拓扑一致，不能把两条腿边
    # 在根部交叉配对。
    points = [
        [-outer_leg, top], [outer_leg, top],
        [outer_leg, inner_bottom], [free_end, inner_bottom],
        [free_end, bottom], [half_inner, bottom],
        [half_inner, inner_top], [-half_inner, inner_top],
        [-half_inner, bottom], [-free_end, bottom],
        [-free_end, inner_bottom], [-outer_leg, inner_bottom],
    ]
    # 外侧腰角、内侧根角、自由端上下、外侧根角、内侧腰角。
    radii_by_vertex = [outer_r1, outer_r1, r2, r3, r4, outer_r2,
                       r1, r1, outer_r2, r4, r3, r2]
    return [polygon(points, radii_by_vertex)]


def build(p):
    p = dict(p)
    hot = bool(p.get("useHotRolled", False))
    contours = _hot_contour(p) if hot else _cold_contour(p)
    ...
    return {
        ...
        **p,
        ...
        "width": width,
        "depth": depth,
        "wallThickness": p.get("wallThickness", 0) if not hot else 0,
        "cornerRadius": p.get("bendRadius", 0) if not hot else p.get("rootRadius1", 0),
        "manufacturingRoute": "HotRolled" if hot else "ColdBent",
        "geometrySource": "idealizedParametric",
        "specification": f"帽钢 / Omega 型钢 {width:g} × {depth:g} mm",
    }
