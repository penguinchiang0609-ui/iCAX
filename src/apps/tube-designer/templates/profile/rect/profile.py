...
def shape(p):
    width, height, wall = p["width"], p["depth"], p["wallThickness"]
    inner_width, inner_height = width - 2 * wall, height - 2 * wall
    offset_x, offset_y = p["innerOffsetX"], p["innerOffsetY"]
    positive(inner_width, inner_height)
    if abs(offset_x) >= wall or abs(offset_y) >= wall:
        raise ValueError("内孔偏心不能达到壁厚")
    outer = [p[f"outerRadius{i}"] for i in range(1, 5)] if p["useOuterRadii"] else [p["cornerRadius"]] * 4
    inner = [p[f"innerRadius{i}"] for i in range(1, 5)] if p["useInnerRadii"] else [p["innerRadius"]] * 4

    def rectangle(width, height, center_x, center_y, radii):
        points = [
        ...
        return corner_polygon(points, radii)

    return [rectangle(width, height, 0, 0, outer),
            rectangle(inner_width, inner_height, offset_x, offset_y, inner)]
...
def build(p):
    ...
    if actual:
        ...
        result={"contours":contours, "width":width, "depth":depth,
                "wallThickness":p["wallThickness"], "cornerRadius":p["cornerRadius"],
                "innerRadius":p["innerRadius"]}
    result["manufacturingRoute"] = "Unspecified"
    result.update({"kind":"rect","_familyParameters":dict(p),"specification":"方矩管"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision","")})
    return result
