...
def shape(p):
    cell, wall, rib = (p[key] for key in ("cellSize", "wallThickness", "ribThickness"))
    positive(cell, wall, rib)
    width = 2 * cell + rib + 2 * wall
    depth = cell + 2 * wall
    center = (cell + rib) / 2
    return [box(width, depth), box(cell, cell, -center), box(cell, cell, center)]
...
def build(p):
    ...
    if actual:
        ...
        result={"contours":contours,"width":width,"depth":depth,
                "wallThickness":p["wallThickness"],"cornerRadius":0}
    result["manufacturingRoute"] = "Unspecified"
    result.update({"kind":"multi-cell","_familyParameters":dict(p),"specification":"二腔方管"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision","")})
    return result
