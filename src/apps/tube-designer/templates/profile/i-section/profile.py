...
PARAMETERS = ("width", "depth", "rootRadius", "wallThickness", "flangeThickness",
              "useHotRolled", "useIndependentFlangeWidths", "topFlangeWidth", "bottomFlangeWidth",
              "webOffset", "useIndependentRadii", "rootRadius1", "rootRadius2",
              "rootRadius3", "rootRadius4", "legEndRadius")


SLOPE = 1.0 / 6.0


def _span(width, web_left, web_right):
    left=web_left+width/2
    right=width/2-web_right
    if min(left,right)<=1e-6:
        raise ValueError("腹板偏距使腹板超出翼板")
    return left,right


def _shape_hot_rolled(p):
    width=float(p["width"])
    depth=float(p["depth"])
    wall=float(p["wallThickness"])
    flange=float(p["flangeThickness"])
    top_width=float(p["topFlangeWidth"] if p.get("useIndependentFlangeWidths",False) else width)
    bottom_width=float(p["bottomFlangeWidth"] if p.get("useIndependentFlangeWidths",False) else width)
    offset=float(p.get("webOffset",0))
    positive(width,depth,wall,flange,top_width,bottom_width)
    if p.get("useIndependentRadii",False):
        roots=[float(p["rootRadius1"]),float(p["rootRadius2"]),
               float(p["rootRadius3"]),float(p["rootRadius4"])]
    else:
        roots=[float(p["rootRadius"])]*4
    toe=float(p.get("legEndRadius",0))
    if any(not math.isfinite(r) or r<0 for r in roots+[toe]):
        raise ValueError("圆角半径不能为负数")
    web_left=offset-wall/2
    web_right=offset+wall/2
    top_left,top_right=_span(top_width,web_left,web_right)
    bottom_left,bottom_right=_span(bottom_width,web_left,web_right)
    slope_length=math.hypot(1,SLOPE)
    fillet_projection=1-SLOPE/slope_length
    for span in (top_left,top_right,bottom_left,bottom_right):
        if flange-SLOPE*span/2<=1e-6:
            raise ValueError("翼板端部厚度必须大于零")
    if any(span<=fillet_projection*(root+toe)+1e-6
           for span,root in ((bottom_right,roots[0]),(top_right,roots[1]),
                             (top_left,roots[2]),(bottom_left,roots[3]))):
        raise ValueError("工字钢根部 R 与腿端 R 覆盖了中间斜直线")

    top_edge=depth/2
    bottom_edge=-depth/2
    top_inner_mid=top_edge-flange
    bottom_inner_mid=bottom_edge+flange
    # flangeThickness is the thickness at the midpoint of each one-sided
    # flange projection.  The inner surface is the national 1:6 slope.
    top_root_y=top_inner_mid-SLOPE*top_right/2
    top_toe_y=top_inner_mid+SLOPE*top_right/2
    top_left_root_y=top_inner_mid-SLOPE*top_left/2
    top_left_toe_y=top_inner_mid+SLOPE*top_left/2
    bottom_root_y=bottom_inner_mid+SLOPE*bottom_right/2
    bottom_toe_y=bottom_inner_mid-SLOPE*bottom_right/2
    bottom_left_root_y=bottom_inner_mid+SLOPE*bottom_left/2
    bottom_left_toe_y=bottom_inner_mid-SLOPE*bottom_left/2
    points=[[-bottom_width/2,bottom_edge],[bottom_width/2,bottom_edge],
            [bottom_width/2,bottom_toe_y],[web_right,bottom_root_y],
            [web_right,top_root_y],[top_width/2,top_toe_y],
            [top_width/2,top_edge],[-top_width/2,top_edge],
            [-top_width/2,top_left_toe_y],[web_left,top_left_root_y],
            [web_left,bottom_left_root_y],[-bottom_width/2,bottom_left_toe_y]]
    corner_radii=[0,0,toe,roots[0],roots[1],toe,0,0,toe,roots[2],roots[3],toe]
    return polygon(points,corner_radii)


def _shape_generic(p):
    width=float(p["width"])
    depth=float(p["depth"])
    wall=float(p["wallThickness"])
    flange=float(p["flangeThickness"])
    ...
    positive(width,depth,wall,flange,top_width,bottom_width)
    if 2*flange>=depth:
        raise ValueError("翼板厚度必须小于总高的一半")
    if abs(offset)+wall/2>=min(top_width,bottom_width)/2:
        raise ValueError("腹板偏距使腹板超出翼板")
    if p.get("useIndependentRadii",False):
        roots=[float(p["rootRadius1"]),float(p["rootRadius2"]),
               float(p["rootRadius3"]),float(p["rootRadius4"])]
        ...
        roots=[float(p["rootRadius"])]*4
    if any(not math.isfinite(r) or r<0 for r in roots):
        raise ValueError("根部 R 不能为负数")
    top_y=depth/2-flange
    bottom_y=-depth/2+flange
    web_left=offset-wall/2
    web_right=offset+wall/2
    points=[[-bottom_width/2,-depth/2],[bottom_width/2,-depth/2],
            [bottom_width/2,bottom_y],[web_right,bottom_y],
            [web_right,top_y],[top_width/2,top_y],
            [top_width/2,depth/2],[-top_width/2,depth/2],
            [-top_width/2,top_y],[web_left,top_y],
            [web_left,bottom_y],[-bottom_width/2,bottom_y]]
    corner_radii=[0,0,0,roots[0],roots[1],0,0,0,0,roots[2],roots[3],0]
    return polygon(points,corner_radii)


def _shape(p):
    return _shape_hot_rolled(p) if p.get("useHotRolled",False) else _shape_generic(p)


def build(p):
    p=dict(p)
    contour=_shape(p)
    width,depth=envelope([contour])
    positive(width,depth)
    return {"contours":[contour],**p,"_parameters":p,"kind":"i-section",
            "width":width,"depth":depth,"wallThickness":p["wallThickness"],
            "cornerRadius":p["rootRadius"],"specification":f"工字钢 {width:g} × {depth:g} mm",
            "manufacturingRoute":"HotRolledI" if p.get("useHotRolled",False) else "Parametric"}
