...
def fitting(section,context):
    ...
    for loops,pose in g.frames(section):
        ...
        for shift in range(12):
            ...
            if abs(w-wb)>t or abs(offset)>t or abs(upper-lower)>t:continue
            rise=y[3]-y[2]
            if rise < -t or abs((y[5]-y[4])-rise)>t:continue
            index=0 if rise<=t else 1
            params=dict(width=w,depth=h,wallThickness=web,geometrySource='idealizedFallback',
                sectionModel='i-hot-parallel' if index==0 else 'i-hot-tapered')
            params.update({f'model{index}FlangeThickness':upper,f'model{index}RootRadius':root,
                           f'model{index}ToeRadius':toe})
            if index:params['model1FlangeSlope']=math.degrees(math.atan2(rise,(w-web)/2))
            return q.result(params,pose)
    return False
