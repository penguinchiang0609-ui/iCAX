...
def _active_lips(p):
    if not p.get("useLips", False):
        return 0.0, 0.0, 90.0, 90.0
    if p.get("useIndependentLips", False):
        return (float(p["topLipLength"]), float(p["bottomLipLength"]),
                float(p["topLipAngle"]), float(p["bottomLipAngle"]))
    length=float(p["lipLength"])
    angle=float(p["lipAngle"])
    return length,length,angle,angle


def _centreline(p):
    h=float(p["depth"])
    top=float(p["topFlangeWidth"])
    bottom=float(p["bottomFlangeWidth"])
    thickness=float(p["wallThickness"])
    positive(h,top,bottom,thickness)
    if h<=thickness:raise ValueError("总高度 H 必须大于板厚 t")
    top_lip,bottom_lip,top_angle,bottom_angle=_active_lips(p)
    for length in (top_lip,bottom_lip):
        if not math.isfinite(length) or length<0:raise ValueError("卷边长度必须为非负有限数值")
    for length,angle in ((top_lip,top_angle),(bottom_lip,bottom_angle)):
        if length>0 and (not math.isfinite(angle) or not 0<angle<180):
            raise ValueError("卷边角度必须大于 0° 且小于 180°")

    half=(h-thickness)/2
    bottom_corner=[-bottom,-half]
    top_corner=[top,half]
    points=[bottom_corner,[0,-half],[0,half],top_corner]
    if bottom_lip>0:
        angle=math.radians(bottom_angle)
        points.insert(0,[bottom_corner[0]+bottom_lip*math.cos(angle),
                         bottom_corner[1]+bottom_lip*math.sin(angle)])
    if top_lip>0:
        angle=math.radians(top_angle)
        points.append([top_corner[0]-top_lip*math.cos(angle),
                       top_corner[1]-top_lip*math.sin(angle)])
    return points


def build(p):
    p=dict(p)
    radius=float(p["bendRadius"])
    if not math.isfinite(radius) or radius<0:raise ValueError("内弯半径 R 必须为非负有限数值")
    contour=strip(_centreline(p),float(p["wallThickness"]),radius)
    width,depth=envelope([contour])
    positive(width,depth)
    return {"contours":[contour],**p,"_parameters":p,"kind":"z-section",
            "width":width,"depth":depth,"wallThickness":float(p["wallThickness"]),
            "cornerRadius":radius,"specification":f"Z 型钢 {width:g} × {depth:g} mm",
            "manufacturingRoute":"ColdFormed","geometrySource":"idealizedParametric"}
