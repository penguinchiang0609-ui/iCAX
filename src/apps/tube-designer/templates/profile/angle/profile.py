...
def polygon(points,corner_radii=None):
    ...
    for i,(start,end,middle,d,r) in enumerate(corners):
        ...
        if math.dist(previous[1],start)>1e-10:
...
def strip(points,thickness,inner_radius,outer_radius=None,free_end_radii=(0,0),inner_offset=(0,0)):
    """Bent strip with independently specified inside and outside bend radii.

    Points are theoretical centreline intersections, not finished outside sizes.
    When outer_radius is omitted, retain the constant-thickness offset rule.
    ``inner_offset`` moves only the material-side/inside contour; the outside
    contour remains anchored to the supplied centreline.  This models an
    eccentric inside boundary instead of translating the whole angle.
    """
    positive(thickness)
    ...
    outer_radius=inner_radius+thickness if outer_radius is None else outer_radius
    if not math.isfinite(outer_radius) or outer_radius<0:raise ValueError("外弯半径不能为负数")
    if (not isinstance(inner_offset,(list,tuple)) or len(inner_offset)!=2
            or any(not math.isfinite(value) for value in inner_offset)):
        raise ValueError("内轮廓偏心距必须为两个有限数")
    ox,oy=inner_offset
    directions=[]
    ...
    for sign in (1,-1):
        vertices=[];rs=[]
        # For this centreline orientation sign > 0 is the material-side
        # contour.  Only that side receives the eccentricity.
        offset_x,offset_y=(ox,oy) if sign>0 else (0,0)
        for i,p in enumerate(points):
            a=directions[max(0,i-1)];b=directions[min(i,len(directions)-1)]
            if i in (0,len(points)-1):
                vertices.append([p[0]+offset_x-sign*b[1]*thickness/2,
                                 p[1]+offset_y+sign*b[0]*thickness/2])
                # Hot-rolled toe radius belongs to the material-side corner
                # of each free end.  ``sign > 0`` is the inside strip for
                # this centreline orientation; the outside end remains sharp.
                rs.append(free_end_radii[0 if i==0 else 1] if sign>0 else 0);continue
            cross=a[0]*b[1]-a[1]*b[0]
            if abs(cross)<1e-10:raise ValueError("折弯中心线相邻段不能共线")
            dot=a[0]*b[0]+a[1]*b[1]
            shift=sign*thickness/2/(1+dot)
            vertices.append([p[0]+offset_x-shift*(a[1]+b[1]),
                             p[1]+offset_y+shift*(a[0]+b[0])])
            rs.append(inner_radius if sign*cross>0 else outer_radius)
...
PARAMETERS = ("width", "depth", "wallThickness", "outerRadius", "mirrorX", "useHotRolled", "useInnerRadius", "innerRadius", "freeEndRadius", "useIndependentFreeEndRadii", "freeEndRadius1", "freeEndRadius2", "innerOffsetX", "innerOffsetY")


def reflect_x(contours):
    """Mirror the complete finished section about the vertical datum axis."""
    result=copy.deepcopy(contours)
    for contour in result:
        if contour["kind"]=="polygon":
            contour["points"]=[[-x,y] for x,y in contour["points"]]
        elif contour["kind"]=="path":
            for edge in contour["segments"]:
                for key in ("start","middle","end","center"):
                    if key in edge:edge[key]=[-edge[key][0],edge[key][1]]
                for key in ("controlPoints","poles"):
                    if key in edge:edge[key]=[[-x,y] for x,y in edge[key]]
                if edge["kind"]=="ellipseArc":
                    edge["rotation"]=math.pi-edge.get("rotation",0)
                    edge["startAngle"],edge["endAngle"]=-edge["startAngle"],-edge["endAngle"]
    return result
...
def build(p):
    ...
    inner_radius=(p["innerRadius"] if p.get("useInnerRadius",False)
                  else max(p["outerRadius"]-p["wallThickness"],0))
    ...
    # The centreline starts at the vertical-leg end and finishes at the
    # horizontal-leg end, while the public parameter order is horizontal,
    # then vertical.
    contours=[strip([[-w/2,h/2],[-w/2,-h/2],[w/2,-h/2]],p["wallThickness"],
              inner_radius,p["outerRadius"],[free_end_radii[1],free_end_radii[0]],
              inner_offset=(dx,dy))]
    if p.get("mirrorX",False):
        contours=reflect_x(contours)
    width,depth=envelope(contours)
    positive(width,depth)
    return {"contours":contours,**p,"_parameters":p,"kind":"angle",
            "width":width,"depth":depth,
            "wallThickness":p.get("wallThickness",0),
            "cornerRadius":0,
            "specification":f"角钢 {width:g} × {depth:g} mm",
            "manufacturingRoute":"HotRolledAngle" if p.get("useHotRolled",False) else "Parametric"}
