...
def build(p):
    """Build the nominal flat-bar contour directly from this template's inputs."""
    p = dict(p)
    width = float(p["width"])
    depth = float(p["depth"])
    ...
    radius = float(p.get("cornerRadius", 0)) if hot else 0.0
    if not math.isfinite(radius) or radius < 0:
        raise ValueError("热轧圆角 R 必须为非负有限数值")
    if hot and radius * 2 >= min(width, depth):
        raise ValueError("热轧圆角 R 不能超过板厚或宽度的一半")

    contours = [box(width, depth, rs=[radius] * 4)]
    ...
    return {
        ...
        **p,
        ...
        "wallThickness": 0,
        "cornerRadius": radius,
        "specification": f"扁钢 {actual_width:g} × {actual_depth:g} mm",
        "manufacturingRoute": "HotRolledFlatBar" if hot else "Parametric",
