...
MODELS = {"flat-bar":"model_0"}
PARAMETERS = {"flat-bar":{"width":"width","depth":"depth","cornerRadius":"cornerRadius"}}

def selected(p):
    name=p.get("sectionModel","flat-bar")
...
def build(p):
    ...
    result["manufacturingRoute"] = {"flat-bar":"Unspecified"}[name]
    result.update({"kind":"rect-bar","_familyParameters":dict(p),"specification":"方矩棒 / 扁钢"+" "+str(result["width"])+" × "+str(result["depth"])+" mm","geometrySource":"providedBoundary" if actual else "idealizedFallback","sourceRevision":p.get("sourceRevision",""),"sectionModel":name})
    return result
