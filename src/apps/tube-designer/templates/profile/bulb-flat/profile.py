...
def shape(p):
    # b and h are the actual theoretical envelope dimensions.  The bulb
    # slope is fixed at 30 degrees from vertical, as required by the
    # parameterized nominal section definition.
    b,h,t,r1,r2,r=[p[k] for k in (
        "width","depth","wallThickness","bulbRadius1","bulbRadius2","endRadius")]
    positive(b,h,t,r1,r2)
    if t>=b: raise ValueError("腹板厚度必须小于球扁钢总宽")
    if 2*r>=t: raise ValueError("自由端圆角必须小于腹板厚度的一半")
    # The upper bulb transition is filleted by r1.  Move the theoretical
    # sloping-line intersection outward so the filleted envelope remains b.
    tip=b+r1*(1/math.tan(math.pi/12)-1)
    drop=(tip-t)*math.tan(math.pi/6)
    if drop>=h: raise ValueError("球头斜线与圆角超过总高")
    pts=[[0,0],[t,0],[t,h-drop],[tip,h],[0,h]]
    return [polygon(pts,[r,r2,r1,r1,r])]
...
def build(parameters):
    ...
    w,h=envelope(loops)
    return {"kind":"bulb-flat","width":w,"depth":h,"wallThickness":p["wallThickness"],"contours":loops,"specification":"球扁钢"+f" {w:g} × {h:g} mm"}
