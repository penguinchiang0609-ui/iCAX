...
def capsule_path(center, width, height):
    positive(width, height)
    if abs(width - height) <= 1e-9:
        return circle_path(center, width / 2)
    cx, cy = center
    if width > height:
        radius, half = height / 2, (width - height) / 2
        bl, br = [cx-half, cy-radius], [cx+half, cy-radius]
        tr, tl = [cx+half, cy+radius], [cx-half, cy+radius]
        segments = [
            {"kind":"line", "start":bl, "end":br},
            {"kind":"arc", "start":br, "middle":[cx+half+radius,cy], "end":tr},
            {"kind":"line", "start":tr, "end":tl},
            {"kind":"arc", "start":tl, "middle":[cx-half-radius,cy], "end":bl},
        ]
    else:
        radius, half = width / 2, (height - width) / 2
        br, tr = [cx+radius, cy-half], [cx+radius, cy+half]
        tl, bl = [cx-radius, cy+half], [cx-radius, cy-half]
        segments = [
            {"kind":"line", "start":br, "end":tr},
            {"kind":"arc", "start":tr, "middle":[cx,cy+half+radius], "end":tl},
            {"kind":"line", "start":tl, "end":bl},
            {"kind":"arc", "start":bl, "middle":[cx,cy-half-radius], "end":br},
        ]
    return {"kind":"path", "closed":True, "segments":segments}
...
def shape(p):
    ...
    contours=[]
    for index,(a,b) in enumerate(((w,h),(w-2*t,h-2*t))):
        center=[0,0] if index==0 else [dx,dy]
        contours.append(capsule_path(center, a, b))
    return contours
