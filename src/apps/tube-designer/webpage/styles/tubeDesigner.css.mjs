...
.tube-parameter-advanced[open] > summary { margin-bottom: 10px; }
.tube-parameter-advanced-item { display: contents; }
.tube-diagram-resize { position:absolute; z-index:20; pointer-events:auto; touch-action:none; }
.tube-diagram-resize.is-n, .tube-diagram-resize.is-s { left:8px; right:8px; height:8px; cursor:ns-resize; }
.tube-diagram-resize.is-n { top:-4px; } .tube-diagram-resize.is-s { bottom:-4px; }
.tube-diagram-resize.is-e, .tube-diagram-resize.is-w { top:8px; bottom:8px; width:8px; cursor:ew-resize; }
.tube-diagram-resize.is-e { right:-4px; } .tube-diagram-resize.is-w { left:-4px; }
.tube-diagram-resize.is-ne, .tube-diagram-resize.is-nw, .tube-diagram-resize.is-se, .tube-diagram-resize.is-sw { width:12px; height:12px; }
.tube-diagram-resize.is-ne { top:-4px; right:-4px; cursor:nesw-resize; }
.tube-diagram-resize.is-sw { bottom:-4px; left:-4px; cursor:nesw-resize; }
.tube-diagram-resize.is-nw { top:-4px; left:-4px; cursor:nwse-resize; }
.tube-diagram-resize.is-se { bottom:-4px; right:-4px; cursor:nwse-resize; }
.tube-floating-parameter-diagram-layer { position:absolute; inset:0; z-index:11; overflow:hidden; pointer-events:none; }
.tube-floating-parameter-diagram-layer > [data-floating-parameter-diagram],
.tube-floating-parameter-diagram-layer > [data-library-floating-diagram] { pointer-events:auto; }
.tube-library-diagram-dock[style*="height:"] { align-items:stretch; }
.tube-library-diagram-dock[style*="height:"] .tube-library-diagram-content { height:auto; max-height:none; }
.tube-floating-parameter-diagram[style*="height:"] .tube-floating-parameter-diagram-body { flex:1; max-height:none; }
.td-profile-annotation[data-parameter-level="advanced"], .tool-diagram-annotation[data-parameter-level="advanced"] { display:none; }
[data-parameter-advanced-open="true"] .td-profile-annotation[data-parameter-level="advanced"], [data-parameter-advanced-open="true"] .tool-diagram-annotation[data-parameter-level="advanced"] { display:initial; }
...
.tube-designer-scene-specification-tree {
  position: absolute;
  top: 56px;
  left: 14px;
  z-index: 11;
  ...
  overflow: hidden;
  border: 1px solid rgba(92, 205, 146, .5);
  border-radius: 4px;
  background: rgba(13, 35, 40, .9);
  box-shadow: 0 8px 22px rgba(3, 18, 23, .3);
  color: #d9eee4;
  pointer-events: auto;
  user-select: none;
}
.tube-designer-scene-specification-tree > header {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 30px;
  align-items: stretch;
  border-bottom: 1px solid rgba(92, 205, 146, .34);
  background: rgba(18, 91, 61, .72);
}
.tube-designer-scene-specification-tree > header .tube-designer-scene-specification-switch {
  border-bottom: 0;
}
.tube-designer-scene-specification-collapse {
  display: grid;
  place-items: center;
  min-width: 30px;
  margin: 0;
  padding: 0;
  border: 0;
  border-left: 1px solid rgba(92, 205, 146, .34);
  background: rgba(8, 42, 38, .36);
  color: #bfead2;
  cursor: pointer;
  font: inherit;
}
.tube-designer-scene-specification-collapse:hover,
.tube-designer-scene-specification-collapse:focus-visible {
  outline: none;
  background: rgba(51, 142, 98, .4);
  color: #fff;
}
.tube-designer-scene-specification-collapse-icon {
  font-size: 21px;
  line-height: 1;
}
.tube-designer-scene-specification-tree.is-collapsed {
  left: 0;
  width: 34px;
  max-height: none;
  border-left: 0;
...
.tube-designer-scene-specification-collapse.is-collapsed {
  display: flex;
  ...
  padding: 6px 0;
  border-left: 0;
  background: rgba(18, 91, 61, .84);
}
.tube-designer-scene-specification-collapse-label {
  writing-mode: vertical-rl;
  font-size: 10px;
  font-weight: 700;
  letter-spacing: 2px;
}
.tube-designer-scene-specification-tree-body {
  max-height: min(52vh, 330px);
  overflow-x: hidden;
  overflow-y: auto;
  scrollbar-width: thin;
}
.tube-designer-scene-specification-switch {
  display: grid;
  ...
  min-height: 30px;
  margin: 0;
  padding: 4px 9px 4px calc(9px + var(--tube-designer-annotation-tree-depth, 0) * 13px);
  border: 0;
  border-bottom: 1px solid rgba(134, 174, 178, .16);
...
.tube-designer-scene-specification-switch:hover,
.tube-designer-scene-specification-switch:focus-visible {
  outline: none;
  background: rgba(51, 142, 98, .28);
  color: #fff;
}
.tube-designer-scene-specification-switch > small {
  color: #8fb2aa;
  font-size: 10px;
}
...
.tube-designer-scene-specification-check {
  position: relative;
  box-sizing: border-box;
  width: 13px;
  height: 13px;
  border: 1px solid #5f9f83;
  border-radius: 2px;
  background: rgba(5, 25, 28, .62);
}
.tube-designer-scene-specification-switch.is-all .tube-designer-scene-specification-check {
  border-color: #51daa0;
  background: #238a60;
}
.tube-designer-scene-specification-switch.is-all .tube-designer-scene-specification-check::after {
  position: absolute;
  top: 0;
  left: 3px;
  width: 4px;
...
.tube-designer-scene-specification-switch.is-mixed .tube-designer-scene-specification-check::after {
  position: absolute;
  top: 5px;
  left: 2px;
  width: 7px;
...
.tube-designer-scene-specification-label {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
...
/* Product editing keeps display configuration and manufacturing planning as
   separate stages. Both stages live in the existing scroll owner so switching
   tabs never replaces the page or resets either sidebar. */
.tube-designer-product-editor-tabs {
  position: sticky;
  top: 0;
  z-index: 5;
  display: grid;
  ...
  min-width: 0;
  padding: 6px 6px 0;
  background: linear-gradient(#f7f9f9 72%, rgba(247, 249, 249, .92));
  border-bottom: 1px solid #c4d1d5;
}
.tube-designer-product-editor-tabs button {
  display: grid;
  min-width: 0;
  min-height: 43px;
...
.tube-designer-product-editor-tabs button + button { border-left: 1px solid #d4dfe2; }
.tube-designer-product-editor-tabs button:hover { background: #edf5f3; }
.tube-designer-product-editor-tabs button.selected {
  color: #137f73;
  border-bottom-color: #15998b;
  background: #e4f2ef;
}
.tube-designer-product-editor-tabs button span {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  color: #294c55;
...
.tube-designer-product-editor-tabs button small {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  color: #789096;
...
.tube-designer-product-diagram > header > div { display: grid; gap: 2px; }
.tube-designer-product-diagram > header strong { color: #294b54; font-size: 12px; }
...
.tube-designer-product-dimension {
  ...
  color: #c2d7da;
  background: #1c3942;
  border: 1px solid #44646c;
  border-radius: 4px;
  cursor: pointer;
  font: inherit;
  text-align: left;
}
.tube-designer-product-dimension:hover,
.tube-designer-product-dimension.is-active {
  color: #fff;
  border-color: #e3a83f;
  box-shadow: inset 3px 0 #e3a83f;
  background: #294650;
}
.tube-designer-product-dimension span {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  color: #8fb0b6;
...
.tube-designer-product-dimension strong {
...
.tube-designer-plan-state { justify-items: start; }
.tube-designer-plan-state strong { color: #294c55; font-size: 14px; }
.tube-designer-plan-state p { margin: 0; line-height: 1.65; font-size: 11px; }
.tube-designer-plan-state.is-loading { grid-template-columns: auto minmax(0, 1fr); align-items: center; }
.tube-designer-plan-state.is-loading p { grid-column: 2; }
.tube-designer-plan-state.is-error { border-color: #d7aa98; background: #fff7f2; color: #985636; }
.tube-designer-plan-state.is-stale { border-color: #d9bd83; background: #fffaf0; }
.tube-designer-plan-state button,
.tube-designer-plan-result button { min-height: 30px; padding: 5px 12px; }
.tube-designer-plan-result > header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 10px;
...
.tube-designer-plan-result > header > div { display: grid; gap: 2px; min-width: 0; }
.tube-designer-plan-result > header strong { color: #294c55; font-size: 14px; }
.tube-designer-plan-result > header span { color: #789096; font-size: 10px; }
.tube-designer-plan-tables { display: grid; gap: 7px; min-width: 0; }
.tube-designer-plan-table { min-width: 0; border: 1px solid #c8d7da; border-radius: 5px; background: #fff; overflow: hidden; }
.tube-designer-plan-table > summary {
  display: flex;
  justify-content: space-between;
  gap: 10px;
  padding: 9px 10px;
...
.tube-designer-plan-table > summary small { color: #748e95; font-weight: 400; }
.tube-designer-plan-table-scroll { max-height: min(42vh, 360px); overflow: auto; scrollbar-gutter: stable; }
.tube-designer-plan-table table { width: max-content; min-width: 100%; border-collapse: collapse; font-size: 10px; }
.tube-designer-plan-table th,
.tube-designer-plan-table td { max-width: 190px; padding: 7px 9px; border-bottom: 1px solid #e1e9eb; text-align: left; white-space: nowrap; }
.tube-designer-plan-table th { position: sticky; top: 0; z-index: 1; color: #668087; background: #f6f9f9; font-weight: 600; }
.tube-designer-plan-table th small { display: block; color: #94a5aa; font-size: 8px; font-weight: 400; }
.tube-designer-plan-note { color: #789096; line-height: 1.55; font-size: 9px; }
...
.tube-designer-config-subsection-content > .tube-designer-user-preset-bar,
.tube-designer-parameter-group-content > .tube-designer-user-preset-bar {
  margin-bottom: 6px;
...
.tube-profile-library-tab-row button.selected { border-color: #57aca3; background: #e1f2ef; color: #11796d; }
.tube-profile-library-tab-row button small { color: inherit; font-size: 9px; font-variant-numeric: tabular-nums; opacity: .78; }
.tube-profile-library-group { overflow: hidden; border: 1px solid #cbd8dc; border-radius: 6px; background: #fff; }
.tube-profile-library-group-heading { display: flex; align-items: center; justify-content: space-between; gap: 6px; width: 100%; min-height: 34px; padding: 8px; border: 0; background: #e9f0f2; color: #294e57; text-align: left; cursor: pointer; }
.tube-profile-library-group-heading:hover { background: #e2ecee; }
.tube-profile-library-group-heading small { flex-shrink: 0; color: #72868c; }
.tube-profile-library-group > div[hidden] { display: none; }
.tube-profile-library-group-content { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); min-width: 0; }
.tube-profile-library-group-content[hidden] { display: none; }
...
.tube-profile-library-card:nth-child(even) { border-right: 0; }
.tube-profile-library-card:hover { background: #eff7f5; }
.tube-profile-library-card.selected { background: #e2f2ef; box-shadow: inset 3px 0 var(--designer-accent); }
.tube-profile-library-card:disabled { cursor: default; opacity: .6; }
.tube-profile-library-card-preview { display: grid; width: 42px; height: 42px; flex: 0 0 42px; place-items: center; overflow: hidden; border: 1px solid #d4e2e4; border-radius: 6px; background: #edf5f5; color: #16867b; }
.tube-profile-library-card-preview .tube-profile-library-svg { width: 37px; height: 37px; filter: none; }
.tube-profile-library-card-preview .tube-profile-library-svg .outer { fill: none; stroke: currentColor; stroke-width: 1.8; vector-effect: non-scaling-stroke; }
.tube-profile-library-card-preview .tube-profile-library-svg .hole { fill: none; stroke: #78aaa6; stroke-width: 1.35; vector-effect: non-scaling-stroke; }
.tube-profile-library-card-preview.is-solid .tube-profile-library-svg .outer { fill: #8fcac4; stroke: currentColor; }
.tube-profile-library-card-preview .tube-profile-library-svg-fallback { fill: none; stroke: currentColor; stroke-width: 1.8; }
.tube-profile-library-card-copy { display: grid; gap: 2px; min-width: 0; }
.tube-profile-library-card-copy strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.tube-profile-library-card-copy strong { font-size: 11px; }
.tube-profile-library-card-copy small { overflow: hidden; color: var(--designer-muted); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
...
.tube-profile-library-editor { display: grid; grid-template-rows: minmax(0, 1fr) auto; align-content: stretch; }
.tube-profile-library-editor-body { display: grid; align-content: start; gap: 12px; min-height: 0; padding: 13px; overflow-x: hidden; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; }
.tube-profile-library-parameter-section { display: grid; gap: 8px; }
.tube-profile-library-parameter-section > header { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.tube-profile-library-parameter-section > header > div { display: grid; gap: 2px; min-width: 0; }
...
.tube-profile-library-parameter-list { display: grid; gap: 6px; }
.tube-profile-library-field-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:4px; }
.tube-profile-library-parameter-group { border:1px solid #c7d6da; background:#f8fbfb; }
.tube-profile-library-parameter-group > summary { padding:7px 8px; color:#39716c; background:#eaf2f2; cursor:pointer; font-weight:600; }
.tube-profile-library-parameter-group > .tube-profile-library-field-grid { padding:6px; }
.tube-profile-library-field-grid > .tube-designer-field { display:flex; align-items:center; gap:4px; padding:4px; border:1px solid #d6e1e5; background:white; min-width:0; margin:0; }
.tube-profile-library-field-grid > .tube-designer-field > span { font-size:11px; flex:1; min-width:0; }
.tube-profile-library-field-grid > .is-line-full { grid-column:1 / -1; }
.tube-profile-library-field-grid > .tube-parameter-advanced-item > .is-line-full { grid-column:1 / -1; }
.tube-profile-library-field-grid > .tube-parameter-advanced-item > .tube-designer-field.is-line-full { grid-column:1 / -1; }
.tube-profile-library-field-grid > .is-string { flex-wrap:wrap; }
.tube-profile-library-field-grid > .is-string > span { flex-basis:100%; }
.tube-profile-library-field-grid > .is-number > input { width:var(--tube-designer-control-preferred-width,72px); min-width:40px; max-width:var(--tube-designer-control-max-width,84px); flex:0 1 var(--tube-designer-control-preferred-width,72px); }
.tube-profile-library-field-grid > .is-choice > select { width:var(--tube-designer-control-preferred-width,200px); flex:0 1 var(--tube-designer-control-preferred-width,200px); }
...
.tube-profile-library-parameter-section .tube-profile-library-diagram-toggle { flex: 0 0 auto; min-height: 24px; padding: 3px 7px; border: 1px solid #b8ced1; border-radius: 3px; appearance: none; background: #f6faf9; color: #356b6a; font: inherit; font-size: 9px; font-weight: 600; cursor: pointer; box-shadow: none; }
.tube-profile-library-parameter-section .tube-profile-library-diagram-toggle:hover { border-color: #78aaa6; background: #edf6f4; color: #27766f; }
.tube-profile-library-parameter-section .tube-profile-library-diagram-toggle:focus-visible { border-color: #198f84; outline: 2px solid #9ed3cd; outline-offset: 1px; }
.tube-profile-library-parameter-diagram { min-width: 0; }
.tube-profile-library-parameter-diagram[hidden] { display: none !important; }
...
.tube-profile-library-preview-section > header { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.tube-profile-library-preview-section > header > div { display: grid; gap: 2px; min-width: 0; }
...
.tube-profile-library-editor-footer { align-self: end; display: flex; align-items: center; justify-content: space-between; gap: 9px; min-height: 58px; box-sizing: border-box; padding: 11px 13px; border-top: 1px solid #cbd5da; background: #fff; }
.tube-profile-library-editor-footer > div { display: flex; flex-wrap: wrap; gap: 7px; }
.tube-profile-library-preview-hud { position:absolute; top:12px; left:12px; z-index:3; display:grid; gap:2px; max-width:min(260px,calc(100% - 24px)); padding:8px 10px; border:1px solid rgba(139,190,193,.42); border-radius:6px; background:rgba(14,35,43,.84); color:#dce9eb; pointer-events:none; backdrop-filter:blur(5px); }
.tube-profile-library-preview-hud strong { overflow:hidden; color:#e1f0f0; font-size:13px; text-overflow:ellipsis; white-space:nowrap; }
.tube-profile-library-preview-hud > span { color:#91b1b6; font-size:10px; }
.tube-profile-library-preview-hud > small { display:none; color:#f0c28c; font-size:10px; line-height:1.4; }
.tube-profile-library-preview-hud.is-busy > small, .tube-profile-library-preview-hud > small.error { display:block; }
...
.tube-profile-library-card:focus-visible,
.tube-profile-library-tab-row button:focus-visible { outline: 2px solid #178f82; outline-offset: 1px; }
.tube-tool-library-panel,
.tube-tool-library-editor { box-sizing: border-box; height: 100%; min-height: 0; overflow: hidden; }
.tube-tool-library-panel { grid-template-rows: auto auto minmax(0, 1fr); }
.tube-tool-library-heading > div { display: grid; gap: 2px; }
.tube-tool-library-filters { display: grid; gap: 8px; padding: 10px; border-bottom: 1px solid #d2dde0; }
.tube-tool-library-filters input { box-sizing: border-box; width: 100%; min-height: 33px; padding: 6px 8px; border: 1px solid #bdcdd2; border-radius: 5px; background: #fff; }
.tube-tool-library-tab-row { display: flex; gap: 5px; min-width: 0; }
.tube-tool-library-tab-row + .tube-tool-library-tab-row { padding-top: 2px; border-top: 1px solid #d2dde0; }
.tube-tool-library-tab-row button { display: inline-flex; flex: 1; align-items: center; justify-content: center; gap: 4px; min-width: 0; padding: 6px 3px; border: 1px solid #c5d4d8; border-radius: 4px; background: #fff; color: #59737b; font-size: 11px; }
.tube-tool-library-tab-row button.selected { border-color: #57aca3; background: #e1f2ef; color: #11796d; }
.tube-tool-library-tab-row button small { color: inherit; font-size: 9px; font-variant-numeric: tabular-nums; opacity: .78; }
.tube-tool-library-list { min-height: 0; padding: 8px; overflow: auto; }
.tube-tool-library-group { margin-bottom: 9px; overflow: hidden; border: 1px solid #cbd8dc; border-radius: 6px; background: #fff; }
.tube-tool-library-group-heading { display: flex; align-items: center; justify-content: space-between; gap: 6px; width: 100%; padding: 8px; border: 0; background: #e9f0f2; color: #294e57; text-align: left; cursor: pointer; }
.tube-tool-library-group-heading:hover { background: #e2ecee; }
.tube-tool-library-group-heading small { flex-shrink: 0; color: #72868c; }
.tube-tool-library-group > div[hidden] { display: none; }
.tube-tool-library-group-items { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); min-width: 0; }
.tube-tool-library-group-items[hidden] { display: none; }
.tube-tool-library-card { display: flex; align-items: center; gap: 9px; width: 100%; min-width: 0; padding: 10px 8px; border: 0; border-top: 1px solid #e0e7e9; border-right: 1px solid #e0e7e9; background: #fff; color: #2b4c55; text-align: left; cursor: pointer; }
.tube-tool-library-card:nth-child(even) { border-right: 0; }
.tube-tool-library-card:hover { background: #eff7f5; }
.tube-tool-library-card.selected { background: #e2f2ef; box-shadow: inset 3px 0 #178f82; }
.tube-tool-library-card-art { display: grid; width: 42px; height: 42px; flex: 0 0 42px; place-items: center; border: 1px solid #d4e2e4; border-radius: 6px; background: #edf5f5; color: #16867b; }
.tube-tool-library-card-art svg { width: 34px; height: 34px; overflow: visible; fill: none; stroke: currentColor; stroke-width: 2.4; stroke-linejoin: round; stroke-linecap: round; }
.tube-tool-library-card-art .tube-profile-library-svg .outer,
.tube-tool-library-card-art .tube-profile-library-svg .hole { fill: none; stroke: currentColor; stroke-width: 2.2; vector-effect: non-scaling-stroke; }
.tube-tool-library-card-copy { display: grid; min-width: 0; gap: 3px; }
.tube-tool-library-card-copy strong, .tube-tool-library-card-copy small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-card-copy strong { font-size: 12px; }
.tube-tool-library-card-copy small { color: #74888f; font-size: 10px; }
.tube-tool-library-empty,
.tube-tool-library-editor-empty { display: grid; place-items: center; align-content: center; gap: 8px; min-height: 180px; padding: 18px; color: var(--designer-muted); text-align: center; }
.tube-tool-library-empty strong { color: var(--designer-ink); }
.tube-tool-library-error { display: grid; gap: 6px; margin: 0 0 8px; padding: 10px; border: 1px solid #e6b8a6; border-radius: 6px; background: #fff4ef; color: #a34f36; font-size: 11px; }
.tube-tool-library-error strong { color: #8e3e29; }
.tube-tool-library-error span { line-height: 1.4; }
.tube-product-template-library-panel { box-sizing: border-box; height: 100%; min-height: 0; overflow: hidden; }
.tube-product-template-library-heading > div { display: grid; gap: 2px; }
.tube-product-template-library-filters { display: grid; gap: 8px; padding: 10px; border-bottom: 1px solid #d2dde0; }
.tube-product-template-library-filters input { box-sizing: border-box; width: 100%; min-height: 33px; padding: 6px 8px; border: 1px solid #bdcdd2; border-radius: 5px; background: #fff; }
.tube-product-template-library-tab-row { display: flex; gap: 5px; min-width: 0; }
.tube-product-template-library-tab-row button { display: inline-flex; flex: 1; align-items: center; justify-content: center; gap: 4px; min-width: 0; padding: 6px 3px; border: 1px solid #c5d4d8; border-radius: 4px; background: #fff; color: #59737b; font-size: 11px; }
.tube-product-template-library-tab-row button.selected { border-color: #57aca3; background: #e1f2ef; color: #11796d; }
.tube-product-template-library-tab-row small { color: inherit; font-size: 9px; font-variant-numeric: tabular-nums; opacity: .78; }
.tube-product-template-library-list { min-height: 0; padding: 8px; overflow: auto; }
.tube-product-template-library-group { margin-bottom: 9px; overflow: hidden; border: 1px solid #cbd8dc; border-radius: 6px; background: #fff; }
.tube-product-template-library-group-heading { display: flex; align-items: center; justify-content: space-between; gap: 6px; width: 100%; padding: 8px; border: 0; background: #e9f0f2; color: #294e57; text-align: left; cursor: pointer; }
.tube-product-template-library-group-heading:hover { background: #e2ecee; }
.tube-product-template-library-group-heading small { flex-shrink: 0; color: #72868c; }
.tube-product-template-library-group > div[hidden] { display: none; }
.tube-product-template-library-group-items { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); min-width: 0; }
.tube-product-template-library-group-items[hidden] { display: none; }
.tube-product-template-library-card { display: flex; align-items: center; gap: 9px; width: 100%; min-width: 0; padding: 10px 8px; border: 0; border-top: 1px solid #e0e7e9; border-right: 1px solid #e0e7e9; background: #fff; color: #2b4c55; text-align: left; cursor: pointer; }
.tube-product-template-library-card:nth-child(even) { border-right: 0; }
.tube-product-template-library-card:hover { background: #eff7f5; }
.tube-product-template-library-card.selected { background: #e2f2ef; box-shadow: inset 3px 0 #178f82; }
.tube-product-template-library-card-art { display: grid; width: 42px; height: 42px; flex: 0 0 42px; place-items: center; border: 1px solid #d4e2e4; border-radius: 6px; background: #edf5f5; color: #16867b; font-size: 25px; line-height: 1; }
.tube-product-template-library-card-art svg { display: block; width: 34px; height: 34px; overflow: visible; fill: none; stroke: currentColor; stroke-linecap: round; stroke-linejoin: round; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-rail { stroke: #2f8f86; stroke-width: 2.4; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-post { stroke: #4aa29a; stroke-width: 2.8; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-large-post { stroke: #197c73; stroke-width: 4.4; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-infill { stroke: #70b8b0; stroke-width: 1.05; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-base { stroke: #5ca69f; stroke-width: 1.8; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-panel { fill: rgba(113, 175, 181, .48); stroke: #5eaaa8; stroke-width: 1.2; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-glass { fill: rgba(113, 194, 223, .22); stroke: #4a9db4; stroke-width: .8; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-spear { fill: #4aa29a; stroke: #4aa29a; stroke-width: .6; }
.tube-product-template-library-card-art .tube-designer-schematic .guardrail-post-cap { fill: #8ac8c2; stroke: #4f9d95; stroke-width: .5; }
.tube-product-template-library-card > span:last-child { display: grid; min-width: 0; gap: 3px; }
.tube-product-template-library-card strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; }
.tube-product-template-library-card small { color: #74888f; font-size: 10px; }
.tube-product-template-library-summary { box-sizing: border-box; height: 100%; min-height: 0; padding: 13px; overflow: auto; }
.tube-product-template-library-summary dl, .tube-product-template-library-meta { display: grid; grid-template-columns: 75px minmax(0, 1fr); gap: 8px; margin: 14px 0; }
.tube-product-template-library-summary dt, .tube-product-template-library-meta dt { color: var(--designer-muted); font-size: 10px; }
.tube-product-template-library-summary dd, .tube-product-template-library-meta dd { margin: 0; overflow-wrap: anywhere; color: #30525b; font-size: 11px; }
.tube-product-template-library-summary p, .tube-product-template-library-editor-body > p { color: var(--designer-muted); font-size: 11px; line-height: 1.5; }
...
.tube-product-template-library-parameter-section,
.tube-product-template-library-parameter-group { min-width: 0; border: 1px solid #cbdadd; border-radius: 5px; background: #f8fbfb; }
.tube-product-template-library-parameter-section > summary,
.tube-product-template-library-parameter-group > summary { display: flex; box-sizing: border-box; min-width: 0; align-items: center; justify-content: space-between; gap: 8px; padding: 8px 9px; color: #355b64; cursor: pointer; list-style: none; user-select: none; }
.tube-product-template-library-parameter-section > summary::-webkit-details-marker,
.tube-product-template-library-parameter-group > summary::-webkit-details-marker { display: none; }
.tube-product-template-library-parameter-section > summary::before,
.tube-product-template-library-parameter-group > summary::before { content: "›"; flex: 0 0 auto; color: #5f7c83; font-size: 16px; line-height: 1; transform: rotate(0deg); transition: transform .12s ease; }
.tube-product-template-library-parameter-section[open] > summary::before,
.tube-product-template-library-parameter-group[open] > summary::before { transform: rotate(90deg); }
.tube-product-template-library-parameter-section > summary > span,
.tube-product-template-library-parameter-group > summary > span { min-width: 0; flex: 1 1 auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.tube-product-template-library-parameter-section > summary > small,
.tube-product-template-library-parameter-group > summary > small { flex: 0 0 auto; color: #789096; font-size: 9px; font-weight: 400; }
.tube-product-template-library-parameter-section > summary { border-radius: 4px; background: #e6f1f0; font-size: 11px; font-weight: 700; }
.tube-product-template-library-parameter-section[open] > summary { border-bottom: 1px solid #cbdadd; border-radius: 4px 4px 0 0; }
.tube-product-template-library-parameter-section-content { display: grid; min-width: 0; gap: 6px; padding: 7px; }
...
.tube-tool-library-editor { display: grid; grid-template-rows: auto minmax(0, 1fr); }
.tube-tool-library-editor-heading { display: flex; align-items: center; justify-content: space-between; gap: 10px; min-width: 0; padding: 11px 13px; border-bottom: 1px solid #bcd7d5; background: #e5f2f1; }
.tube-tool-library-editor-title { display: flex; align-items: center; gap: 9px; min-width: 0; }
.tube-tool-library-editor-title > div { display: grid; min-width: 0; gap: 3px; }
.tube-tool-library-editor-title strong { overflow: hidden; color: #294e57; font-size: 14px; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-editor-title span:not(.tube-tool-library-editor-art) { overflow: hidden; color: var(--designer-muted); font-size: 10px; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-editor-art { display: grid; width: 38px; height: 38px; flex: 0 0 38px; place-items: center; border: 1px solid #cbdcde; border-radius: 5px; background: #e7f2f1; color: #16867b; }
.tube-tool-library-editor-art svg { display: block; width: 30px; height: 30px; fill: none; stroke: currentColor; stroke-width: 2.3; stroke-linecap: round; stroke-linejoin: round; }
.tube-tool-library-editor-art .tube-profile-library-svg .outer,
.tube-tool-library-editor-art .tube-profile-library-svg .hole { fill: none; stroke: currentColor; stroke-width: 2.1; vector-effect: non-scaling-stroke; }
...
.tube-tool-library-tube-section > header span { color: var(--designer-muted); font-size: 9px; }
.tube-tool-library-tube-header-actions { display: flex !important; align-items: center; justify-content: flex-end; gap: 6px; min-width: 0; }
.tube-tool-library-diagram-toggle { min-height: 24px; padding: 3px 7px; border: 1px solid #9fc7c4; border-radius: 4px; background: #f7fbfb; color: #277b74; font-size: 10px; cursor: pointer; white-space: nowrap; }
.tube-tool-library-diagram-toggle:hover, .tube-tool-library-diagram-toggle:focus-visible { border-color: #198f84; background: #e4f3f1; outline: none; }
.tube-tool-library-section-badge { max-width: 45%; overflow: hidden; color: #177b71 !important; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-tube-parameter-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 10px; }
.tube-tool-library-mold-section { display: grid; gap: 10px; padding: 10px; border: 1px solid #bcd8d5; border-radius: 6px; background: #edf6f5; }
.tube-tool-library-mold-section > header { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
.tube-tool-library-mold-section > header > div { display: grid; min-width: 0; gap: 3px; }
.tube-tool-library-mold-section > header strong { color: #2b5f66; font-size: 13px; }
.tube-tool-library-mold-section > header span { color: var(--designer-muted); font-size: 9px; }
.tube-tool-library-mold-parameters { display: grid; gap: 8px; padding: 9px; border: 1px solid #c9d8dc; border-radius: 5px; background: #fff; }
.tube-tool-library-mold-parameters > header { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
.tube-tool-library-mold-parameters > header strong { color: #35545e; font-size: 12px; }
.tube-tool-library-mold-parameters > header span { flex-shrink: 0; color: var(--designer-muted); font-size: 9px; }
.tube-tool-library-parameter-diagram { display: grid; gap: 8px; padding: 10px; border: 1px solid #c9d8dc; border-radius: 6px; background: #f8fbfb; }
.tube-tool-library-mold-section > .tube-tool-library-parameter-diagram { border-color: #c9d8dc; background: rgba(255, 255, 255, .7); }
.tube-tool-library-mold-section > .tube-tool-library-branch-section { border-color: #c9d8dc; background: #fff; }
.tube-tool-library-mold-section > .tube-tool-library-branch-section > header { padding-bottom: 2px; border-bottom: 1px solid #e0eaeb; }
.tube-tool-library-parameter-diagram > header { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
...
.tube-tool-library-parameter-editor header { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
.tube-tool-library-summary header span,
.tube-tool-library-parameter-editor header span { flex-shrink: 0; color: var(--designer-muted); font-size: 9px; }
.tube-tool-library-summary strong { color: #2b5f66; font-size: 13px; }
.tube-tool-library-summary p { margin: 0; color: var(--designer-muted); font-size: 10px; line-height: 1.5; }
.tube-tool-library-summary span,
.tube-tool-library-summary small,
.tube-tool-library-note,
.tube-tool-library-provenance { color: var(--designer-muted); font-size: 10px; line-height: 1.5; }
.tube-tool-library-meta { display: grid; grid-template-columns: 92px minmax(0, 1fr); gap: 7px 10px; margin: 0; }
.tube-tool-library-meta dt { color: var(--designer-muted); font-size: 10px; }
.tube-tool-library-meta dd { margin: 0; overflow-wrap: anywhere; color: #30525b; font-size: 11px; }
.tube-tool-library-parameter-editor { display: grid; gap: 8px; padding: 10px; border: 1px solid #c9d8dc; border-radius: 6px; background: #fff; }
.tube-tool-library-parameter-editor header strong { color: #35545e; font-size: 12px; }
.tube-tool-library-parameter-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px 10px; }
.tube-tool-library-parameter-grid label { display: grid; gap: 4px; min-width: 0; }
.tube-tool-library-parameter-grid label > span { overflow: hidden; color: #55737b; font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-parameter-grid :is(input, select) { box-sizing: border-box; width: 100%; min-width: 0; min-height: 29px; padding: 4px 6px; border: 1px solid #bdcdd2; border-radius: 4px; background: #fff; color: #2b4c55; font: inherit; font-size: 11px; }
.tube-tool-library-parameter-grid .tube-tool-library-parameter-check { display: flex; align-items: center; gap: 6px; min-height: 29px; padding: 4px 6px; border: 1px solid #d4e1e3; border-radius: 4px; background: #f7fafb; }
.tube-tool-library-parameter-grid .tube-tool-library-parameter-check input { width: auto; min-height: auto; }
.tube-tool-library-fixed-card { display: grid; gap: 4px; padding: 10px; border: 1px solid #d0dfe2; border-radius: 6px; background: #f7fafb; }
.tube-tool-library-fixed-card strong { color: #35545e; font-size: 12px; }
.tube-tool-library-fixed-card span { color: var(--designer-muted); font-size: 10px; line-height: 1.5; }
.tube-tool-library-note { display: grid; gap: 3px; margin: 0; padding: 9px; border-left: 3px solid #438f87; background: #e9f3f1; color: #3e6d69; }
.tube-tool-library-note strong { color: #2f6f69; font-size: 11px; }
.tube-tool-library-note span { color: #557b78; }
.tube-tool-library-provenance { margin: 0; padding-top: 10px; border-top: 1px solid #d4dfe2; }
.tube-tool-library-hud { position: absolute; top: 12px; left: 12px; z-index: 4; display: grid; gap: 4px; width: fit-content; max-width: calc(100% - 165px); padding: 11px 14px; border: 1px solid #36545d; border-radius: 6px; background: rgba(13, 34, 42, .92); color: #d6e8ea; pointer-events: none; }
.tube-tool-library-hud strong { overflow: hidden; font-size: 14px; text-overflow: ellipsis; white-space: nowrap; }
.tube-tool-library-hud span { color: #a8c0c6; font-size: 11px; }
.tube-tool-library-hud small { color: #7d9ca6; font-size: 10px; }
.tube-tool-library-hud p { margin: 2px 0 0; color: #ffd0af; font-size: 10px; }
.tube-tool-library-hud button { pointer-events: auto; }
.tube-tool-library-preview-progress { width: 100%; height: 4px; margin-top: 4px; overflow: hidden; border-radius: 6px; background: #33545d; }
.tube-tool-library-preview-progress > i { display: block; width: 36%; height: 100%; background: #3dc6af; animation: tube-designer-export-indeterminate 1.05s ease-in-out infinite; }
.tube-tool-library-tab-row button:focus-visible,
.tube-tool-library-card:focus-visible { outline: 2px solid #178f82; outline-offset: 1px; }
...
/* Multi-part connection library.  A connection composes existing single-part
   mould operations, assembly motion and locking semantics. */
...
.tube-connection-library-cards { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); }
.tube-connection-library-cards[hidden] { display:none; }
.tube-connection-library-card { display:flex; min-width:0; min-height:72px; align-items:center; gap:8px; padding:8px; border:0; border-right:1px solid #d9e3e5; border-bottom:1px solid #d9e3e5; background:#fff; color:#294e57; text-align:left; cursor:pointer; }
.tube-connection-library-card:nth-child(even) { border-right:0; }
...
.tube-connection-library-card strong { overflow:hidden; font-size:10px; text-overflow:ellipsis; white-space:nowrap; }
.tube-connection-library-card small { overflow:hidden; color:#73888e; font-size:8px; line-height:1.35; text-overflow:ellipsis; }
.tube-connection-library-card-art { display:grid; width:38px; height:38px; flex:0 0 38px; place-items:center; border:1px solid #d4e2e4; border-radius:5px; background:#edf5f5; color:#16867b; }
.tube-connection-library-card-art svg { width:31px; height:31px; fill:none; stroke:currentColor; stroke-width:2.2; stroke-linecap:round; stroke-linejoin:round; }
...
.tube-connection-library-editor-heading b { flex:0 0 auto; padding:4px 7px; border:1px solid #bcd8d5; border-radius:4px; background:#e1f2ef; color:#177b71; font-size:9px; }
...
.tube-connection-library-intro { margin:0; padding:8px 9px; border-left:3px solid #438f87; background:#e9f3f1; color:#476d6a; font-size:10px; line-height:1.55; }
.tube-connection-library-parameter-section,
.tube-connection-library-block { display:grid; gap:8px; padding:9px; border:1px solid #c9d8dc; border-radius:5px; background:#fff; }
.tube-connection-library-parameter-section.basic > header,
.tube-connection-library-block > header { display:flex; align-items:baseline; justify-content:space-between; gap:8px; }
.tube-connection-library-parameter-section header strong,
.tube-connection-library-block header strong { color:#35545e; font-size:12px; }
.tube-connection-library-parameter-section header small,
.tube-connection-library-block header span { color:#73888e; font-size:9px; }
...
.tube-connection-library-role-list,
.tube-connection-library-operation-list,
.tube-connection-library-steps { display:grid; gap:6px; margin:0; padding:0; list-style:none; }
.tube-connection-library-role-list article { display:flex; align-items:center; gap:8px; padding:7px; border:1px solid #d9e4e6; border-radius:4px; background:#f7fafb; }
.tube-connection-library-role-list i { display:grid; width:22px; height:22px; flex:0 0 22px; place-items:center; border-radius:50%; background:#dcefeb; color:#197b72; font-size:10px; font-style:normal; font-weight:700; }
.tube-connection-library-role-list article > div,
.tube-connection-library-operation-list article > div { display:grid; min-width:0; gap:2px; }
.tube-connection-library-role-list strong,
.tube-connection-library-operation-list strong { color:#35545e; font-size:10px; }
.tube-connection-library-role-list small,
.tube-connection-library-operation-list small { color:#74888f; font-size:8px; line-height:1.4; }
.tube-connection-library-operation-list article { display:grid; grid-template-columns:58px minmax(0,1fr); gap:7px; padding:7px; border:1px solid #d7e2e4; border-radius:4px; background:#f8fbfb; }
.tube-connection-library-operation-list article > span { align-self:start; padding:3px 5px; border-radius:3px; background:#e1f2ef; color:#177b71; font-size:9px; text-align:center; }
.tube-connection-library-operation-list dl { display:grid; grid-column:1/-1; grid-template-columns:repeat(2,minmax(0,1fr)); gap:4px; margin:0; }
.tube-connection-library-operation-list dl > div { display:flex; min-width:0; justify-content:space-between; gap:5px; padding:4px 5px; background:#fff; color:#6e858b; font-size:8px; }
.tube-connection-library-operation-list dd { margin:0; color:#2f6863; overflow-wrap:anywhere; }
.tube-connection-library-reuse-note { margin:0; color:#667e84; font-size:9px; line-height:1.5; }
.tube-connection-library-steps { counter-reset:connection-step; }
.tube-connection-library-steps li { display:flex; align-items:flex-start; gap:8px; }
.tube-connection-library-steps li::before { counter-increment:connection-step; content:counter(connection-step); display:grid; width:19px; height:19px; flex:0 0 19px; place-items:center; border-radius:50%; background:#dcefeb; color:#197b72; font-size:9px; }
.tube-connection-library-steps i { min-width:42px; padding:3px 5px; border-radius:3px; background:#f0f5f5; color:#55737b; font-size:8px; font-style:normal; text-align:center; }
.tube-connection-library-steps span { display:grid; gap:2px; }
.tube-connection-library-steps strong { color:#35545e; font-size:9px; }
.tube-connection-library-steps small { color:#74888f; font-size:8px; }
.tube-connection-library-block header button { border:1px solid #b8ced1; border-radius:3px; background:#f6faf9; color:#356b6a; font-size:8px; cursor:pointer; }
.tube-connection-library-output-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:5px; }
.tube-connection-library-output-grid span { display:grid; gap:2px; padding:6px; background:#f6f9fa; color:#667e84; font-size:8px; line-height:1.4; }
.tube-connection-library-output-grid b { color:#35545e; font-size:9px; }
.tube-connection-library-hud { position:absolute; top:12px; left:12px; z-index:4; display:grid; gap:3px; padding:10px 13px; border:1px solid #36545d; border-radius:6px; background:rgba(13,34,42,.92); color:#d6e8ea; pointer-events:none; }
.tube-connection-library-hud strong { font-size:14px; }
.tube-connection-library-hud span { color:#a8c0c6; font-size:10px; }
.tube-connection-library-hud small { color:#7d9ca6; font-size:9px; }
.tube-connection-library-stage { position:absolute; z-index:3; top:50%; left:50%; display:grid; width:min(620px,calc(100% - 70px)); max-height:calc(100% - 120px); grid-template-rows:auto minmax(0,1fr) auto; overflow:hidden; border:1px solid #6ea7a2; border-radius:7px; background:#f5f9f9; box-shadow:0 15px 38px rgba(0,0,0,.28); transform:translate(-50%,-50%); }
.tube-connection-library-stage > header { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:11px 14px; border-bottom:1px solid #b9d5d3; background:#e5f2f1; color:#2c5c61; }
.tube-connection-library-stage > header strong { font-size:13px; }
.tube-connection-library-stage > header span { font-size:9px; }
.tube-connection-library-stage > svg { display:block; box-sizing:border-box; width:100%; min-height:260px; max-height:430px; padding:18px; background:#edf5f5; }
.tube-connection-library-stage svg g { fill:rgba(87,164,155,.25); stroke:#268c83; stroke-width:2.2; }
.tube-connection-library-stage svg .part-b { fill:rgba(98,129,154,.18); stroke:#577b8b; }
.tube-connection-library-stage svg .hole { fill:#edf5f5; stroke:#cc7a2d; }
.tube-connection-library-stage svg .motion { fill:none; stroke:#d27625; stroke-width:2.5; stroke-dasharray:7 5; }
.tube-connection-library-stage svg .weld { fill:none; stroke:#d3652b; stroke-width:6; stroke-dasharray:5 4; }
.tube-connection-library-stage svg marker path { fill:#d27625; stroke:none; }
.tube-connection-library-stage svg text { fill:#4b6971; stroke:none; font:12px "Segoe UI","Microsoft YaHei",sans-serif; }
.tube-connection-library-stage > footer { display:flex; flex-wrap:wrap; align-items:center; justify-content:center; gap:8px; padding:10px; border-top:1px solid #d0dfe1; background:#fff; color:#57737a; font-size:9px; }
.tube-connection-library-stage footer span { display:flex; align-items:center; gap:5px; }
.tube-connection-library-stage footer i { display:grid; width:18px; height:18px; place-items:center; border-radius:50%; background:#dcefeb; color:#197b72; font-style:normal; }
.tube-connection-library-stage footer .result { padding:4px 7px; border-radius:3px; background:#e1f2ef; color:#177b71; font-weight:700; }
...
.td-profile-parameter-legend { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1px 4px; padding: 5px; min-width: 0; }
.td-profile-parameter-advanced-divider { grid-column:1 / -1; display: flex; align-items: center; gap: 8px; margin: 7px 6px 2px; color: #39716c; font-size: 11px; font-weight: 600; }
.td-profile-parameter-advanced-divider::before, .td-profile-parameter-advanced-divider::after { content: ''; height: 1px; flex: 1; background: #b9d0d0; }
.td-profile-parameter-advanced-divider span { flex: 0 0 auto; }
.td-profile-parameter-diagram .td-profile-parameter-legend-row { display: grid; grid-template-columns: 21px minmax(0, 1fr); align-items: start; gap: 6px; min-width: 0; width: 100%; padding: 5px 7px; border: 1px solid transparent; border-radius: 4px; background: transparent; color: #365b64; text-align: left; cursor: pointer; }
...
.td-profile-parameter-legend-row > span { display: grid; gap: 1px; min-width: 0; }
...
.tube-component-library-template-body { padding: 6px; }
.tube-component-library-template-body > .tube-component-library-group:last-child { margin-bottom: 0; }
.tube-component-library-group-items { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); min-width: 0; }
.tube-component-library-group-items[hidden] { display: none; }
.tube-component-library-card { display: flex; align-items: center; gap: 9px; width: 100%; min-width: 0; padding: 10px 8px; border: 0; border-top: 1px solid #e0e7e9; border-right: 1px solid #e0e7e9; background: #fff; color: #2b4c55; text-align: left; cursor: pointer; }
.tube-component-library-card:nth-child(even) { border-right: 0; }
...
.tube-component-library-icon .tube-profile-library-svg .outer,
.tube-component-library-icon .tube-profile-library-svg .hole { fill: none; stroke: currentColor; stroke-width: 1.8; vector-effect: non-scaling-stroke; }
...
@media (max-width: 500px) {
  .tube-profile-library-list,
  .tube-component-library-group-items,
  .tube-tool-library-group-items { grid-template-columns: minmax(0, 1fr); }
  .tube-tool-library-parameter-grid { grid-template-columns: minmax(0, 1fr); }
  .tube-tool-library-tube-parameter-grid { grid-template-columns: minmax(0, 1fr); }
  .tube-component-library-card,
  .tube-tool-library-card { border-right: 0; }
}
...
.tube-designer-excel-column-head,.tube-designer-excel-column-row { display: grid; grid-template-columns: 62px minmax(120px,.72fr) minmax(150px,.82fr) minmax(210px,1.2fr) 70px minmax(210px,1fr); align-items: center; gap: 10px; padding: 9px 12px; border-bottom: 1px solid #e0eaec; }
...
.tube-designer-excel-column-alias { min-width: 0; }
...
.tube-designer-excel-head-check { display:inline-flex; align-items:center; gap:6px; white-space:nowrap; }
.tube-designer-excel-include,
.tube-designer-excel-required { justify-content:center; }
...
.tube-library-diagram-dock { position:absolute; z-index:6; top:145px; right:0; width:360px; height:405px; max-height:calc(100% - 16px); display:flex; flex-direction:column; align-items:stretch; box-sizing:border-box; overflow:hidden; max-width:calc(100% - 16px); background:#f6faf9; pointer-events:none; will-change:transform; transform:translateZ(0); }
.tube-library-diagram-dock[hidden] { display:none; }
.tube-library-diagram-actions { display:flex; flex-wrap:wrap; gap:8px; margin-bottom:12px; }
.tube-library-diagram-drag { flex:0 0 auto; display:flex; align-items:center; justify-content:space-between; gap:8px; padding:7px 9px; background:#e4efee; border:1px solid #8abcb6; color:#285f58; cursor:move; touch-action:none; user-select:none; pointer-events:auto; }
.tube-library-diagram-drag strong { font-size:12px; }
...
.tube-library-diagram-content .tube-tool-library-parameter-diagram { margin:0; border:0; border-radius:0; padding:8px; }
.tube-library-diagram-content .td-profile-parameter-diagram > header,
.tube-library-diagram-content .tube-tool-library-parameter-diagram > header { color:#285f58; }
.tube-designer-parameter-panel .tube-designer-field-grid { grid-template-columns:repeat(2, minmax(0, 1fr)); gap:4px; padding:4px; }
...
.tube-designer-parameter-panel .tube-designer-field.is-choice {
  grid-template-columns:minmax(64px, 1fr) minmax(56px, max-content);
  column-gap:5px;
...
.tube-designer-parameter-panel .tube-designer-field.is-number > input { justify-self:start; max-width:84px; }
...
.tube-designer-product-tool-field > [data-tool-parameter-scope] { min-width:0; container:product-tool-parameters / inline-size; }
.tube-product-tool-diagram-open { justify-self:start; }
.tube-floating-parameter-diagram { position:absolute; z-index:12; width:360px; height:405px; max-width:100%; max-height:100%; background:#f4f8f8; border:1px solid #a9bec3; border-radius:5px; box-shadow:0 5px 20px #0005; pointer-events:auto; display:flex; flex-direction:column; will-change:transform; transform:translateZ(0); }
.tube-floating-parameter-diagram > header { display:flex; align-items:center; justify-content:space-between; gap:8px; padding:8px 10px; background:#e4efee; color:#28565d; cursor:move; touch-action:none; user-select:none; border-bottom:1px solid #c6d7da; }
.tube-floating-parameter-diagram > header strong { font-size:12px; }
.tube-floating-parameter-diagram > header button { border:0; background:transparent; color:#385c64; cursor:pointer; font-size:20px; line-height:20px; }
.tube-floating-parameter-diagram-body { min-height:0; max-height:380px; overflow:auto; padding:10px; }
.tube-floating-parameter-diagram-body > small { display:block; color:#6a858d; margin-bottom:8px; }
.tube-floating-parameter-diagram .tube-tool-library-diagram-art { min-height:0; }
.tube-designer-product-tool-field [data-tool-parameter-scope] > .tube-designer-field-grid { min-width:0; }
.tube-designer-parameter-panel .tube-designer-product-tool-field [data-tool-parameter-scope] .tube-designer-boolean-field { grid-template-columns:minmax(0, 1fr) auto; }
.tube-designer-parameter-panel .tube-designer-product-tool-field [data-tool-parameter-scope] .tube-designer-field:not(.tube-designer-boolean-field) {
  grid-template-columns:minmax(0, 1fr);
  align-items:start;
}
.tube-designer-product-tool-field [data-tool-parameter-scope] .tube-designer-field > span { min-width:0; overflow-wrap:anywhere; }
.tube-designer-product-tool-field [data-tool-parameter-scope] .tube-designer-field > :is(input:not([type="checkbox"]), select) { max-width:100%; }
@container product-tool-parameters (max-width:279px) {
  .tube-designer-product-tool-field [data-tool-parameter-scope] > .tube-designer-field-grid { grid-template-columns:minmax(0, 1fr); }
}
