...
export function exposeLaserCamAutomation(context, view, commands) {
  ...
  window.__icaxLaser3DCAM.getTubeDesignerProfileLibraryState = () => {
    const systemProfiles = (Array.isArray(view.tubeDesignerSystemProfiles)
      ? view.tubeDesignerSystemProfiles : [])
      .map((profile) => ({ ...profile, scope: "system" }));
    const userProfiles = (Array.isArray(view.tubeDesignerUserData?.profiles)
      ? view.tubeDesignerUserData.profiles : [])
      .map((profile) => ({ ...profile, scope: "user" }));
    const profiles = [...systemProfiles, ...userProfiles];
    const selectedProfileKey = String(view.tubeDesignerSelectedProfileId ?? "");
    const selectedProfile = profiles.find((profile) =>
      `${profile.scope}:${String(profile?.id ?? "")}` === selectedProfileKey)
      ?? profiles.find((profile) => String(profile?.id ?? "") === selectedProfileKey)
      ?? null;
    const profileName = (profile) => {
      const localized = profile?.descriptor?.displayName;
      return String(profile?.name ?? profile?.previewProfile?.name
        ?? localized?.["zh-CN"] ?? localized?.zh ?? localized?.["en-US"] ?? localized?.en ?? "");
    };
    const preview = view.tubeDesignerProfilePreview ?? null;
    const request = view.tubeDesignerProfilePreviewRequest ?? null;
    const applied = view.viewport?.getAppliedViewState?.() ?? null;
    return {
      activeAreaId: view.activeAreaId ?? "",
      pending: Boolean(view.pending),
      progress: view.progress ? {
        title: String(view.progress.title ?? ""),
        stage: String(view.progress.stage ?? ""),
      } : null,
      tubeDesignerLoading: Boolean(view.tubeDesignerLoading),
      tubeDesignerLoaded: Boolean(view.tubeDesignerLoaded),
      userDataLoading: Boolean(view.tubeDesignerUserDataLoading),
      userDataLoaded: Boolean(view.tubeDesignerUserDataLoaded),
      loadProgress: view.tubeDesignerLoadProgress ? {
        title: String(view.tubeDesignerLoadProgress.title ?? ""),
        stage: String(view.tubeDesignerLoadProgress.stage ?? ""),
      } : null,
      selectedProfileId: String(selectedProfile?.id ?? ""),
      selectedProfileKey,
      systemProfileCount: systemProfiles.length,
      userProfileCount: userProfiles.length,
      profiles: profiles.map((profile) => ({
        id: String(profile?.id ?? ""),
        scope: profile.scope,
        selectionKey: `${profile.scope}:${String(profile?.id ?? "")}`,
        entityId: `${profile.scope}:${String(profile?.id ?? "")}`,
        name: profileName(profile),
        profileType: String(profile?.profileType ?? profile?.kind
          ?? (profile.scope === "system" ? "parametric-package" : "")),
      })),
      previewKey: String(preview?.key ?? ""),
      requestKey: String(request?.key ?? ""),
      geometryResourceId: String(preview?.response?.geometryResourceId ?? ""),
      geometryResourceVersion: Number(preview?.response?.geometryResourceVersion ?? 0),
      appliedView: applied ? {
        revision: String(applied.revision ?? ""),
        entityIds: [...(applied.entityIds ?? [])].map(String),
      } : null,
