...
namespace iCAX
{
    namespace Geom
    {
        ...
        * @brief 二维圆角矩形
        * @details
        * RoundRect3 表示一个定义在局部坐标系内的二维矩形，其四个角为圆角。
        * - 中心在局部坐标原点
        * - 可以设置宽度、高度和圆角半径
        ...
        * [7,8)	左下圆角	270°→360°
        */
        class _GEOMETRY_EXP RoundRect3 final : public MltBndCurve3
        {
        public:
            /**
            * @brief 构造函数
            ...
            */
            RoundRect3(IN const Point3& ptCenter_, IN const double& nWidth_, IN const double& nHeight_, IN const double& nRadius_, IN const bool& bCCW_ = true, IN const Dir3& dirNormal_ = Dir3::Up(), IN const Dir3& dirRight_ = Dir3::Right());
