...
//!< 构造函数
iCAX::Geom::MltBndCurve3::MltBndCurve3(IN const CoordSys3& csLocal_)
    : BndCurve3(csLocal_)
    , m_lstSegments()
    , m_lstParams()
{
...
//!< 默认析构函数
iCAX::Geom::MltBndCurve3::~MltBndCurve3()
{
}


//!< 段列表
const std::vector<std::shared_ptr<iCAX::Geom::SglBndCurve3>>& iCAX::Geom::MltBndCurve3::Segments() const
{
    return m_lstSegments;
}

//!< 顶点列表
const std::vector<Point3> iCAX::Geom::MltBndCurve3::GetVertices() const
{
    std::vector<Point3> _lstVerts;
...
//!< 解析u得到段以及段内参数
void iCAX::Geom::MltBndCurve3::ResolveU2Segment(IN const double& nU_, IN const bool& bLeft_, OUT std::shared_ptr<SglBndCurve3>& pSeg_, OUT double& nLocalU_) const
{
...
//!< 获取曲线起始参数
double iCAX::Geom::MltBndCurve3::StartParam() const
{
    return m_lstParams.empty() ? 0.0 : m_lstParams.front().first;
}

//!< 获取曲线终止参数
double iCAX::Geom::MltBndCurve3::EndParam() const
{
    return m_lstParams.empty() ? 0.0 : m_lstParams.back().second;
}

//!< 在给定参数处计算点坐标
Point3 iCAX::Geom::MltBndCurve3::Evaluate(IN const double& nU_) const
{
    std::shared_ptr<SglBndCurve3> _pSeg = nullptr;
    double _nLocalU = .0;
...
//!< 反转
void iCAX::Geom::MltBndCurve3::Reverse()
{
...
//!< 是否在平面上
bool iCAX::Geom::MltBndCurve3::IsOnPlane(IN const Plane3& pln_) const
{
    for (size_t i = 0; i < m_lstSegments.size(); i++)
    {
        if (!m_lstSegments[i]->IsOnPlane(pln_))
        {
            return false;
        }
    }
...
//!< 重算参数表
void iCAX::Geom::MltBndCurve3::ReCalcParamTable()
{
    // 构建累加参数表
    m_lstParams.clear();
    double _nAcc = 0.0;
