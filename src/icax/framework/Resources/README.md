...
它负责资源条目、资源对象池、轻量元信息、持久化策略、manifest 查询、ResourceLoader 抽象和 ResourceLoader 注册表。具体文件格式解析、GPU 上传或业务生命周期调度由插件或产品项目实现。

## 目录结构

- `ResourceKey.*`：Resources 内部资源索引，常规上层代码不需要手动构造。
- `ResourceInfo.h`：资源条目元信息和持久化策略。
- `ResourceLibrary.*`：Project 级资源库入口，提供 `resources.Load<T>(source)`、`resources.Get<T>(source)`、`resources.Unload(source)` 等 API，未来由 `project.Resources()` 暴露。
...
- `ResourceLoaderRegistry.*`：静态资源加载器注册表，提供 `ICAX_REGISTER_RESOURCE_LOADER(ResourceClass, LoaderType)` 宏，按 C++ 资源类型查找 loader 并返回加载结果。
