...
namespace
{
    bool HasDynamicProperty(IN const iCAX::Database::CComponentBase& Component_, IN const std::string& strPropertyName_)
    {
        auto _vecNames = Component_.GetPropertyNameArray();
        return std::find(_vecNames.begin(), _vecNames.end(), strPropertyName_) != _vecNames.end();
    }
}
...
iCAX::Data::PropertyValue iCAX::Database::CDerivedPropertyContext::GetProperty(IN const CPropertyKey& Key_)
{
    ...
    else if (_pMeta->HasTypeByName(Key_.ComponentClass))
    {
    ...
    if (!_bKnownProperty && !HasDynamicProperty(*_pComponent, Key_.PropertyName))
    {
        return iCAX::Data::PropertyValue::Nil;
    }
