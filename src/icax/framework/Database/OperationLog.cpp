...
namespace
{
    ...
    {
        // 未注册组件视为动态组件，字段默认保留，避免破坏脚本或外部动态数据。
...
iCAX::Database::COperationBatchBuilder::COperationBatchBuilder(IN EChangeScopeKind Kind_, IN const std::string& strName_)
{
    m_Batch.Kind = Kind_;
...
{
    if (m_Batch.Kind == EChangeScopeKind::LoadBaseline)
    {
        return;
    }
...
iCAX::Database::COperationBatch iCAX::Database::MakeOperationBatchFromRepositoryEvent(IN const RepositoryEventArgs& Args_, IN EChangeScopeKind Kind_, IN const std::string& strName_)
{
    COperationBatch _Result;
    _Result.Kind = Kind_;
...
iCAX::Database::COperationBatch iCAX::Database::FilterTransactionalOperationBatch(IN const COperationBatch& Batch_)
{
    return FilterTransactionalOperationBatch(Batch_, *GetGlobalMetaRegistry());
}
...
iCAX::Database::COperationBatch iCAX::Database::FilterPersistentOperationBatch(IN const COperationBatch& Batch_)
{
    return FilterPersistentOperationBatch(Batch_, *GetGlobalMetaRegistry());
}
...
void iCAX::Database::COperationBatchJournal::Open(IN const std::string& strPath_, IN const bool bTruncate_)
{
    Close();
...
std::vector<iCAX::Database::COperationBatch> iCAX::Database::COperationBatchJournal::ReadAll(IN const std::string& strPath_) const
{
