...
void iCAX::Database::CMetaRegistrationCatalog::Register(IN ReplayFunc Func_)
{
    Register(std::move(Func_), nullptr);
}

void iCAX::Database::CMetaRegistrationCatalog::Register(IN ReplayFunc Func_, IN const void* pModuleAddress_)
{
    if (!Func_)
    {
        throw std::invalid_argument("Meta registration replay function cannot be empty");
    }
...
void iCAX::Database::CMetaRegistrationCatalog::ReplayAll(IN IMetaRegistry& Registry_)
{
    (void)ReplayFrom(0, Registry_);
}

size_t iCAX::Database::CMetaRegistrationCatalog::ReplayFrom(IN size_t nFirstIndex_, IN IMetaRegistry& Registry_)
{
...
void iCAX::Database::CMetaRegistrationCatalog::ReplayByModulePaths(
    IN IMetaRegistry& Registry_,
...
size_t iCAX::Database::CMetaRegistrationCatalog::Count()
{
    std::lock_guard<std::mutex> _Lock(GetRegistrationMutex());
    return GetRegistrations().size();
}

std::vector<iCAX::Database::CMetaRegistrationCatalog::RegistrationRecord>& iCAX::Database::CMetaRegistrationCatalog::GetRegistrations()
{
