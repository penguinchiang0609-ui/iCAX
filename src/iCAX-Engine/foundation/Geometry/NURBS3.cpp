...
//!< 构造函数
NURBS3::NURBS3(IN const std::vector<Point3>& lstControlPoints_,
...
//!< 获取控制点
const std::vector<Point3>& NURBS3::ControlPoints() const
{
    return m_lstControlPoints;
}

//!< 获取权重
const std::vector<double>& NURBS3::Weights() const
{
    return m_lstWeights;
}

//!< 获取节点向量
const std::vector<double>& NURBS3::KnotVector() const
{
    return m_lstKnots;
}

//!< 获取曲线阶数
int NURBS3::Degree() const
{
    return m_nDegree;
}

//!< 起始参数
double NURBS3::StartParam() const
{
    return m_lstKnots[m_nDegree];
}

//!< 终止参数
double NURBS3::EndParam() const
{
...
//!< 计算曲线上指定参数点
Point3 NURBS3::Evaluate(IN const double& u_) const
{
    if (m_lstControlPoints.empty()) return Point3(0, 0, 0);
    ...
    for (int i = 0; i <= n; i++)
    {
        ...
        numeratorZ += wN * m_lstControlPoints[i].Z();
        denominator += wN;
    }
    ...
    Point3 ptLocal(numeratorX / denominator, numeratorY / denominator, numeratorZ / denominator);
...
//!< 获取曲线所在平面
std::optional<Plane3> iCAX::Geom::NURBS3::SuggestPlane() const
{
    return Plane3(m_lstControlPoints[0], m_lstControlPoints[1], m_lstControlPoints[2]);
}

//!< 是否在平面上
bool iCAX::Geom::NURBS3::IsOnPlane(IN const Plane3& pln_) const
{
...
//!< 缩放和剪切
void NURBS3::ScaleAndShear(IN const Double3& nScale_, IN const Double6& nShear_, IN const bool& bMirrorX_)
{
...
//!< 克隆
std::shared_ptr<Geometry> NURBS3::Clone() const
{
    return std::make_shared<NURBS3>(*this);
}
