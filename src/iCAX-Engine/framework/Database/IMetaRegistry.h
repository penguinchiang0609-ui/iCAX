...
namespace iCAX
{
    namespace Database
    {
        ...
        */
        ...
        /*
        * @brief 创建元数据注册表
        * @return 新的空 IMetaRegistry 实例。
        * @details
        *   调用方应显式将模块注册记录 Replay 到该实例。
        *   Database 不维护全局 MetaRegistry，避免跨产品或跨项目共享数据 schema。
        */
        _DATABASE_EXP std::shared_ptr<IMetaRegistry> CreateMetaRegistry();

        /*
        * @brief 从组件所属 Repository 解析元数据注册表
        * @throws std::runtime_error 组件未挂载到 Repository 或 Repository 未提供 MetaRegistry 时抛出。
        */
