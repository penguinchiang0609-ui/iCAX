    ...
    namespace Database
    {
        ...
        */
        struct _DATABASE_EXP CRepositoryOperation final
        {
            ...
            std::optional<bool> PreviousEnabled;                                           //!< 操作前组件启用状态；组件状态操作和移除组件时使用。
            std::optional<bool> NewEnabled;                                                //!< 操作后组件启用状态；组件状态操作和添加组件时使用。
        ...
        */
        ...
        {
            EOperationBatchKind Kind = EOperationBatchKind::UserCommand;        //!< 批次语义，如普通用户命令、事务、回放等。
        ...
        */
        ...
        public:
            ...
            */
            COperationBatchBuilder(IN EOperationBatchKind Kind_, IN const std::string& strName_);
        ...
        */
        COperationBatch _DATABASE_EXP MakeOperationBatchFromRepositoryEvent(IN const RepositoryEventArgs& Args_, IN EOperationBatchKind Kind_ = EOperationBatchKind::UserCommand, IN const std::string& strName_ = std::string());
        ...
        */
        CChangeSet _DATABASE_EXP BuildChangeSetFromOperationBatch(IN const COperationBatch& Batch_);
        ...
        */
        COperationBatch _DATABASE_EXP FilterPersistentOperationBatch(IN const COperationBatch& Batch_, IN const IMetaRegistry& Meta_);
        ...
        */
        COperationBatch _DATABASE_EXP MakeInverseOperationBatch(IN const COperationBatch& Batch_, IN const std::string& strName_ = std::string());
        ...
        */
        iCAX::Data::Variant _DATABASE_EXP OperationBatchToVariant(IN const COperationBatch& Batch_);
        ...
        * @return 还原后的操作批次。
        */
        COperationBatch _DATABASE_EXP OperationBatchFromVariant(IN const iCAX::Data::Variant& Value_);
        ...
        * @return 字符串形式的序列化结果。
        */
        std::string _DATABASE_EXP SerializeOperationBatch(IN const COperationBatch& Batch_);
        ...
        * @return 还原后的操作批次。
        */
        COperationBatch _DATABASE_EXP DeserializeOperationBatch(IN const std::string& Text_);
        ...
        */
        ...
        public:
            ...
            * @param [in] nVersion_ 期望日志格式版本，由上层产品或文件模块传入；不做迁移，只做严格匹配。
            */
            void Open(
            ...
            * @param [in] nVersion_ 期望日志格式版本；不匹配时抛出异常。
            * @return 按文件顺序读取到的操作批次数组。
            */
            std::vector<COperationBatch> ReadAll(
                IN const std::string& strPath_,
                IN const std::string& strMagic_,
