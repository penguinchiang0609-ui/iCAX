#include "pch.h"
#include "SDORegistrationCatalog.h"
...
void iCAX::Interaction::CSDORegistrationCatalog::Register(IN ReplayFunc Func_)
{
    Register(std::move(Func_), nullptr);
}

void iCAX::Interaction::CSDORegistrationCatalog::Register(IN ReplayFunc Func_, IN const void* pModuleAddress_)
{
    if (!Func_)
    {
        throw std::invalid_argument("SDO registration replay function cannot be empty");
    }
...
void iCAX::Interaction::CSDORegistrationCatalog::ReplayAll(IN CSDORegistry& Registry_)
{
    (void)ReplayFrom(0, Registry_);
}

size_t iCAX::Interaction::CSDORegistrationCatalog::ReplayFrom(
...
void iCAX::Interaction::CSDORegistrationCatalog::ReplayByModulePaths(
    IN CSDORegistry& Registry_,
    IN const std::vector<std::string>& ModulePaths_)
{
    ...
    // ProductRuntime 只回放自己已加载模块贡献的 SDO，避免不同产品互相看见对方的交互面。
    for (const auto& _Registration : _Registrations)
    {
...
size_t iCAX::Interaction::CSDORegistrationCatalog::Count()
{
    std::lock_guard<std::mutex> _Lock(GetRegistrationMutex());
    return GetRegistrations().size();
}

std::vector<iCAX::Interaction::CSDORegistrationCatalog::RegistrationRecord>&
iCAX::Interaction::CSDORegistrationCatalog::GetRegistrations()
{
