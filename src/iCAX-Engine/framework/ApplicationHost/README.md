...
ApplicationHost 持有应用级共享对象：`CServiceProvider` 和 `CMailChannelRegistry`。前者只管理 Service，后者只管理 mail channel；二者不混放。BehaviourRegistry、CommandRegistry、产品级 MetaRegistry 和产品级 ResourceLoaderRegistry 由 `ProductRuntime` 创建并按产品模块路径回放注册；Project 只拿到明确注入的产品/项目依赖，不共享项目数据。
