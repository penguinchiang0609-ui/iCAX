...
CommandTarget 的入口会直接收到 `ApplicationContext`、可选的 `ProductContext` 和可选的 `ProjectContext`。应用级命令没有产品/项目上下文，产品级命令没有项目上下文，项目级命令三者都有。命令需要服务、Repository、资源库或 PDO 时，从对应的产品/项目上下文获取。
