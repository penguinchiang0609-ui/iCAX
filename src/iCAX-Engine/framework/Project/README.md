...
- `ProjectSDO.h`：项目级公共 SDO 方法常量，当前包含 `Project.GetState`、`Project.Undo`、`Project.Redo`、`Project.GetUndoRedoState`。这些方法操作项目数据，但调用时要求 SDO 来自具体 Scene SDO，并同时携带 ProjectContext 和 SceneContext。
- `SceneRuntimeScheduler.*`：Scene 运行时调度器，负责帧时间统计、目标帧间隔和下一帧唤醒时间；Scene 每帧把调度器产生的 `deltaTime/totalTime` 传给 Universe。
- `ProjectCatalog.*`：一个项目目录，拥有自己的 `catalogId/name/path`，负责打开、关闭和查询目录内的主 Project，不负责 Tick；Scene 线程由 ProjectScene 自己持有。Catalog 构造时必须拿到完整上下文、注册表、SDO channel 注册表和资源加载器注册表工厂。
