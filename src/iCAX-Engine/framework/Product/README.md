...
ProductRuntime 保存自己的 `productChannelId`，并通过应用级 `IMailChannelService` 显式创建和删除产品级 `CMailChannel`。前端通过 ApplicationHost 启动产品后，拿到该产品的 frontend post office，再与产品级命令入口对话。
