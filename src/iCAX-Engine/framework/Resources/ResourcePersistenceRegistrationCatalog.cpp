...
namespace
{
    ...
    std::string NormalizeModulePath(IN const std::string& strPath_)
    {
        ...
        const auto _RequiredLength =
            GetFullPathNameA(strPath_.c_str(), 0, nullptr, nullptr);
        ...
        std::vector<char> _Buffer(
            static_cast<size_t>(_RequiredLength) + 1);
        ...
        std::replace(
            _Normalized.begin(), _Normalized.end(), '/', '\\');
        std::transform(
            _Normalized.begin(),
            _Normalized.end(),
            _Normalized.begin(),
            [](IN const unsigned char ch_) {
    ...
    std::vector<std::string> NormalizeModulePaths(
        IN const std::vector<std::string>& ModulePaths_)
    {
        ...
        for (const auto& _Path : ModulePaths_)
        {
            ...
            if (!_Normalized.empty() &&
                std::find(
                    _Result.begin(), _Result.end(), _Normalized) ==
                    _Result.end())
            {
...
void iCAX::Resource::CResourcePersistenceRegistrationCatalog::Register(
    IN ReplayFunc Func_)
{
    ...
    IN const void* pModuleAddress_)
{
    if (!Func_)
    {
        throw std::invalid_argument(
            "Resource persistence registration replay function cannot be empty");
    }

    std::lock_guard<std::mutex> _Lock(GetRegistrationMutex());
    GetRegistrations().push_back({
        GetModulePathFromAddress(pModuleAddress_),
        std::move(Func_)
    });
}

void iCAX::Resource::CResourcePersistenceRegistrationCatalog::ReplayAll(
    IN CResourceLibrary& Library_)
{
    (void)ReplayFrom(0, Library_);
}

size_t iCAX::Resource::CResourcePersistenceRegistrationCatalog::ReplayFrom(
    ...
    IN CResourceLibrary& Library_)
{
    ...
    {
        std::lock_guard<std::mutex> _Lock(GetRegistrationMutex());
        const auto& _All = GetRegistrations();
        nFirstIndex_ = (std::min)(nFirstIndex_, _All.size());
        _Registrations.assign(
            _All.begin() + nFirstIndex_, _All.end());
        nFirstIndex_ = _All.size();
    }

    for (const auto& _Registration : _Registrations)
    {
        _Registration.Replay(Library_);
    }
    return nFirstIndex_;
}

void iCAX::Resource::CResourcePersistenceRegistrationCatalog::
ReplayByModulePaths(
    IN CResourceLibrary& Library_,
    IN const std::vector<std::string>& ModulePaths_)
{
    ...
    for (const auto& _Registration : _Registrations)
    {
        if (std::find(
            _ModulePaths.begin(),
            _ModulePaths.end(),
            _Registration.ModulePath) != _ModulePaths.end())
        {
            _Registration.Replay(Library_);
        }
    }
...
size_t iCAX::Resource::CResourcePersistenceRegistrationCatalog::Count()
{
    std::lock_guard<std::mutex> _Lock(GetRegistrationMutex());
    return GetRegistrations().size();
}

std::vector<iCAX::Resource::CResourcePersistenceRegistrationCatalog::
RegistrationRecord>&
iCAX::Resource::CResourcePersistenceRegistrationCatalog::GetRegistrations()
{
