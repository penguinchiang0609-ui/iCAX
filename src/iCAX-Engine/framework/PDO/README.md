...
PDO 采用双方预先约定的固定二进制布局，不做运行时字段自描述。普通命令走 Facades/JSON；PDO 面向高频数据，CEF 场景下可通过 `CSharedPDOArena` 使用 Windows OS shared memory 暴露给 renderer，再由 JS 侧包装成 TypedArray 读取。PDO 允许丢帧，但读侧必须通过 `CPDOReadLease` 声明读租约，写侧不会覆盖正在读取的 buffer。
