...
Service 在整个 Application 内共享，但通信通道不进入 ServiceProvider。`ApplicationHost` 直接持有应用级 `CMailChannelRegistry`，并显式注入 ProductRuntime / Project / Scene；PDO 由 Scene 自己持有并通过 `ISceneContext::PDOHub()` 暴露。ComponentMeta、Behaviour、ResourceLoader 和 CommandHandler 按 ProductRuntime 隔离；`Universe` 实例、Repository、资源对象、Scene ResourceLoaderRegistry、Scene 邮箱、Scene PDO 和 Scene 线程全部按 Scene 隔离。
