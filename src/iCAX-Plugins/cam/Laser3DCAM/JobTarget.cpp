...
namespace
{
    class CJobTarget final : public iCAX::Command::CCommandTarget
    {
    public:
        CJobTarget()
            : CCommandTarget("Job")
        {
            Bind("Get", &iCAX::CAM::Commands::HandleGetJob);
            Bind("SetMachine", &iCAX::CAM::Commands::HandleSetJobMachine);
        }
    };

    static_assert(iCAX::Command::IsStatelessCommandTargetType<CJobTarget>);
}

ICAX_REGISTER_COMMAND_TARGET(CJobTarget)
...
namespace
{
        ...
        IN const std::shared_ptr<iCAX::CAM::CJobComponent>& pJob_)
    {
        ...
        if (pJob_)
        {
            ...
            _Job["machineLoaded"] = _pMachine && _pMachine->GetEnabled() && !_pMachine->GetMachineResourceID().empty();
        }
        else
        {
    ...
    iCAX::Command::CCommandResponse MakeJobResponse(IN iCAX::Database::IRepository& Repository_)
    {
        auto _Jobs = _CollectEntitiesWithComponent<iCAX::CAM::CJobComponent>(Repository_);
...
iCAX::Command::CCommandResponse HandleGetJob(
...
iCAX::Command::CCommandResponse HandleSetJobMachine(
