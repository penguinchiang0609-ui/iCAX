...
namespace
{
    class CWorkpieceFacade final : public iCAX::Interaction::CFacade
    {
    public:
        CWorkpieceFacade()
            : CFacade("Workpiece")
        {
            ExposeMethod("Instantiate", &iCAX::CAM::Facades::HandleInstantiateWorkpiece);
            ExposeMethod("List", &iCAX::CAM::Facades::HandleListWorkpieces);
            ExposeMethod("SetActive", &iCAX::CAM::Facades::HandleSetActiveWorkpiece);
        }
    };

    static_assert(iCAX::Interaction::IsStatelessFacadeType<CWorkpieceFacade>);
}

ICAX_REGISTER_FACADE(CWorkpieceFacade)
