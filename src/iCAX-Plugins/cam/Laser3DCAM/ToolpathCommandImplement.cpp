...
namespace
{
    ...
    {
        auto _pRoot = _GetComponent<iCAX::CAM::CRootComponent>(Repository_);
        if (!_pRoot || _pRoot->GetActiveMachineID().is_nil())
        {
            return {};
        }

        auto _pEntity = Repository_.GetEntity(_pRoot->GetActiveMachineID());
        auto _pMachine = _GetComponent<iCAX::CAM::CMachineComponent>(_pEntity);
    ...
    std::pair<std::shared_ptr<iCAX::Database::IEntity>, std::shared_ptr<iCAX::CAM::CMachineComponent>> _GetActiveOrFirstMachine(
        IN iCAX::Database::IRepository& Repository_)
    {
        auto _Active = _GetActiveMachine(Repository_);
        if (_Active.first && _Active.second)
        {
            return _Active;
        }

        auto _Machines = _CollectEntitiesWithComponent<iCAX::CAM::CMachineComponent>(Repository_);
        if (_Machines.empty())
        {
            return {};
        }
        ...
        auto _pRoot = _GetOrCreateComponent<iCAX::CAM::CRootComponent>(Repository_);
        _SetUuidProperty(_pRoot, iCAX::CAM::CRootComponent::PropertyName_ActiveMachineID, _Machines.front().first->GetID());
        return _Machines.front();
    }
