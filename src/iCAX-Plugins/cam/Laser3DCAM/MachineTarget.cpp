...
namespace
{
    class CMachineTarget final : public iCAX::Command::CCommandTarget
    {
    public:
        CMachineTarget()
            : CCommandTarget("Machine")
        {
            Bind("Instantiate", &iCAX::CAM::Commands::HandleInstantiateMachine);
            Bind("List", &iCAX::CAM::Commands::HandleListMachines);
            Bind("GetElement", &iCAX::CAM::Commands::HandleGetMachineElement);
            Bind("SetElementTransform", &iCAX::CAM::Commands::HandleSetMachineElementTransform);
            Bind("SetElementAppearance", &iCAX::CAM::Commands::HandleSetMachineElementAppearance);
            Bind("SetJointLimits", &iCAX::CAM::Commands::HandleSetMachineJointLimits);
            Bind("SetParameters", &iCAX::CAM::Commands::HandleSetMachineParameters);
            Bind("SetTCP", &iCAX::CAM::Commands::HandleSetMachineTCP);
            Bind("SetToolTCP", &iCAX::CAM::Commands::HandleSetMachineToolTCP);
            Bind("SetEnabled", &iCAX::CAM::Commands::HandleSetMachineEnabled);
    ...
    static_assert(iCAX::Command::IsStatelessCommandTargetType<CMachineTarget>);
}

ICAX_REGISTER_COMMAND_TARGET(CMachineTarget)
...
namespace
{
    ...
    void SetDefaultJobMachineIfEmpty(
        IN iCAX::Database::IRepository& Repository_,
        IN const iCAX::Data::uuid& MachineID_)
    {
        ...
        {
            (void)_pEntity;
            if (_pJob && _pJob->GetMachineEntityID().is_nil())
            {
                _SetUuidProperty(_pJob, iCAX::CAM::CJobComponent::PropertyName_MachineEntityID, MachineID_);
                return;
            }
            if (_pJob)
            {
        ...
        IN const iCAX::Data::uuid& MachineID_)
    {
        ...
        {
            (void)_pEntity;
            if (_pJob && _pJob->GetMachineEntityID() == MachineID_)
            {
                _SetUuidProperty(_pJob, iCAX::CAM::CJobComponent::PropertyName_MachineEntityID, iCAX::Data::uuid());
                _SetStringProperty(_pJob, iCAX::CAM::CJobComponent::PropertyName_Status, "NeedMachine");
            }
        }
    }
    ...
    iCAX::Command::CCommandResponse MakeMachineListResponse(IN iCAX::Project::ISceneContext& Scene_)
    {
        ObjectMap _Result;
    ...
    iCAX::Command::CCommandResponse MakeMachineResponse(
...
iCAX::Command::CCommandResponse HandleListMachines(
...
iCAX::Command::CCommandResponse HandleGetMachineElement(
...
iCAX::Command::CCommandResponse HandleSetMachineElementTransform(
    IN const iCAX::Command::CCommandRequest& Request_,
    IN iCAX::Application::IApplicationContext&,
    IN iCAX::Product::IProductContext*,
    IN iCAX::Project::IProjectContext*,
...
iCAX::Command::CCommandResponse HandleSetMachineElementAppearance(
    IN const iCAX::Command::CCommandRequest& Request_,
    IN iCAX::Application::IApplicationContext&,
    IN iCAX::Product::IProductContext*,
    IN iCAX::Project::IProjectContext*,
...
iCAX::Command::CCommandResponse HandleSetMachineJointLimits(
    IN const iCAX::Command::CCommandRequest& Request_,
    IN iCAX::Application::IApplicationContext&,
    IN iCAX::Product::IProductContext*,
    IN iCAX::Project::IProjectContext*,
...
iCAX::Command::CCommandResponse HandleInstantiateMachine(
    IN const iCAX::Command::CCommandRequest &Request_,
    IN iCAX::Application::IApplicationContext &,
    ...
    IN iCAX::Project::ISceneContext *pSceneContext_)
{
    ...
    auto _Payload = _DecodeObjectPayload(Request_);
    auto& _Repository = _Scene.Database();
    iCAX::Data::uuid _MachineDefinitionID;
    std::shared_ptr<iCAX::CAM::CMachineDefinitionComponent> _pMachineDefinition;
    ...
    {
        _MachineDefinitionID = _ParseRequiredUuid(_MachineDefinitionIDText, "machineDefinitionId");
        auto [_pDefinitionEntity, _pDefinition] = _FindMachineDefinition(_Repository, _MachineDefinitionID);
        (void)_pDefinitionEntity;
        _pMachineDefinition = _pDefinition;
        if (!_pMachineDefinition)
        {
            throw std::runtime_error("Cam Machine.Instantiate machine definition is not found: " + _MachineDefinitionIDText);
        }
        if (!_pMachineDefinition->GetEnabled())
        {
            throw std::runtime_error("Cam Machine.Instantiate machine definition is disabled: " + _MachineDefinitionIDText);
        }
    }

    auto _MachineResourceID = _pMachineDefinition->GetMachineResourceID();
    if (_MachineResourceID.empty())
    {
        throw std::runtime_error("Cam Machine.Instantiate machine definition has no resource: " + _MachineDefinitionIDText);
    }

    auto _pDescription = _Scene.Resources().Get<iCAX::CAM::CMachineDescriptionResource>(_MachineResourceID);
    if (!_pDescription)
    {
        throw std::runtime_error("Cam Machine.Instantiate machine definition resource is not loaded: " + _MachineResourceID);
    }

    auto _SourcePath = _GetOptionalString(_Payload, "sourcePath", _pDescription->SourcePath);
    if (_SourcePath.empty())
    {
        const auto _Info = _Scene.Resources().GetInfo(_MachineResourceID);
        if (_Info)
        {
    ...
    const auto _ModelName = _pDescription->ModelName.empty() ? _GetDisplayNameFromPath(_SourcePath) : _pDescription->ModelName;
    const auto _DefinitionName = _pMachineDefinition ? _pMachineDefinition->GetName() : std::string();
    ...
    const auto _DescriptionLinearJogStep = GetDescriptionPluginDouble(
        *_pDescription,
        "linear_jog_step_mm",
        GetDescriptionPluginDouble(*_pDescription, "linearJogStep", _pKinematics->GetLinearJogStep()));
    const auto _DescriptionAngularJogStep = GetDescriptionPluginDouble(
        *_pDescription,
        "angular_jog_step_deg",
        GetDescriptionPluginDouble(*_pDescription, "angularJogStep", _pKinematics->GetAngularJogStep()));
    ...
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_MachineDefinitionID, _UuidToString(_MachineDefinitionID));
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_MachineResourceID, _MachineResourceID);
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_DescriptionFormat, _pDescription->SourceFormat);
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_DescriptionVersion, _pDescription->SourceFormatVersion);
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_ModelName, _pDescription->ModelName);
    _SetStringProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_WorkstationName, "默认工位");
    _SetBoolProperty(_pMachine, iCAX::CAM::CMachineInstanceComponent::PropertyName_StaticModel, _pDescription->bStaticModel);
    ...
    _InstantiateMachineStructureFromResource(_Scene, _pMachineEntity, _pMachine);
    SetDefaultJobMachineIfEmpty(_Repository, _pMachineEntity->GetID());
    _SetStringProperty(
        _pStatus,
        iCAX::CAM::CMachineStatusComponent::PropertyName_LastCheckResult,
        "机器定义已实例化："
            + std::to_string(iCAX::CAM::CountMachinePartElements(*_pDescription)) + " parts / "
            + std::to_string(iCAX::CAM::CountMachineJoints(*_pDescription)) + " joints / "
            + std::to_string(iCAX::CAM::CountMachineVisuals(*_pDescription)) + " visuals / "
            + std::to_string(iCAX::CAM::CountMachineCollisions(*_pDescription)) + " collisions");
    _Undo->End();
    return MakeMachineResponse(_Scene, _pMachineEntity, _pMachine);
}

iCAX::Command::CCommandResponse HandleSetMachineEnabled(
    IN const iCAX::Command::CCommandRequest& Request_,
    IN iCAX::Application::IApplicationContext&,
    IN iCAX::Product::IProductContext*,
    IN iCAX::Project::IProjectContext*,
    IN iCAX::Project::ISceneContext* pSceneContext_)
{
    ...
    if (!_bEnabled)
    {
        ClearJobMachineIfMatches(_Repository, _pMachineEntity->GetID());
    }
    else
    {
        SetDefaultJobMachineIfEmpty(_Repository, _pMachineEntity->GetID());
...
iCAX::Command::CCommandResponse HandleSetMachineToolTCP(
    IN const iCAX::Command::CCommandRequest& Request_,
    IN iCAX::Application::IApplicationContext&,
    IN iCAX::Product::IProductContext*,
    IN iCAX::Project::IProjectContext*,
    ...
    IN iCAX::Project::ISceneContext *pSceneContext_)
{
    ...
    _SetStringProperty(_pStatus, iCAX::CAM::CMachineStatusComponent::PropertyName_Status, (_pWorkpieceEntity && _pWorkpiece) ? "Ready" : "NeedWorkpiece");
    _SetStringProperty(_pStatus, iCAX::CAM::CMachineStatusComponent::PropertyName_LastCheckResult, _Result);
    return MakeMachineResponse(_Scene, _pMachineEntity, _pMachine);
}
} // namespace Commands
