...
`SRenderMaterialData` 是资源池中的材质源数据。业务组件、机床 visual、工件实例等只保存 `MaterialResourceID`，不直接保存 diffuse/ambient/specular/emissive 等散字段。`RenderResource` 将几何和材质编码成有版本的 FlatBuffer 资源，前端通过 View 投影得到 URL 后直接读取。这里不表达 shader、贴图、PBR、GPU 参数或具体渲染器状态。
...
- `RenderResource.h/.cpp`：前端几何/材质资源的编码与识别。
- `Schema/`：资源池二进制格式定义。
