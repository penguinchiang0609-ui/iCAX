...
const state = {
  ...
  newProjectError: "",
  newProjectDialogPosition: null,
  newProjectDialogDrag: null,
  bottomDockHeight: 64,
...
const actions = {
  ...
  async openProjectFromPath(projectPath, productId = state.selectedProductId) {
    ...
    await track("App.OpenProjectFile", async () => {
      ...
      state.savedProjectIds.add(state.activeProjectProxy.projectId);
  ...
  async removeRecentProject(projectPath, productId) {
    await track("Product.RemoveRecentProject", async () => {
      const product = await actions.ensureProductStarted(productId);
      await product.removeRecentProject(projectPath);
      await actions.refresh();
    });
  },
  ...
  async chooseProjectFile() {
    ...
    await track("选择项目文件", async () => {
      const extensions = [...new Set((state.appState?.products ?? [])
        .flatMap(getProjectFileExtensions).map((extension) => extension.slice(1)))];
      const path = await bridge.openFileDialog({
        title: "打开项目",
        filters: [{ name: "项目文件", extensions: extensions.length ? extensions : ["icax"] }],
      });
      if (path) await actions.openProjectFromPath(path);
    });
  },
    ...
    await track("Project.Save", async () => {
      ...
      if (saveAs || !state.savedProjectIds.has(project.projectId) || !path || path.includes("://")) {
        if (typeof state.bridge?.saveFileDialog !== "function") {
          throw new Error("当前宿主不支持保存文件选择，请重启更新后的软件。");
        }
        const extensions = getProjectFileExtensions(product).map((extension) => extension.slice(1));
        path = await state.bridge.saveFileDialog({
          title: saveAs ? "项目另存为" : "保存项目",
          defaultPath: path && !path.includes("://") ? path : makeDefaultProjectPath(project.state.projectName, product),
  ...
  async executeRibbonCommand(commandId) {
    if (commandId === "app.save" || commandId === "app.saveAs") {
      await actions.saveProject(commandId === "app.saveAs");
      return;
    }
    if (commandId === "app.open") {
...
// Suppress browser navigation/print/source menus without stopping application
// contextmenu handlers (for example finishing a sketch command).
...
root.addEventListener("click", (event) => {
  ...
  if (action === "refresh") {
    ...
    runAction(() => actions.removeRecentProject(target.dataset.projectPath, target.dataset.productId));
...
root.addEventListener("keydown", (event) => {
  ...
  const target = event.target instanceof Element ? event.target.closest("[data-action]") : null;
  if (!target || target.dataset.action !== "open-project") {
    return;
  }
...
// The startup new-project popup is owned by AppShell, so give it the same
// draggable title-bar behavior as product-owned dialogs.
root.addEventListener("pointerdown", (event) => {
  ...
  const titleBar = target?.closest(".new-project-dialog > header[data-window-dialog-drag]");
  if (!titleBar || target.closest("button,input,select,textarea,a,[data-no-window-drag]")) {
    return;
  }

  const dialog = titleBar.closest(".new-project-dialog");
  if (!dialog) {
    return;
  }
  ...
  titleBar.dataset.windowDragging = "true";
  titleBar.setPointerCapture?.(event.pointerId);
});

root.addEventListener("pointermove", (event) => {
  const active = state.newProjectDialogDrag;
  ...
  const left = Math.max(margin, Math.min(event.clientX - active.offsetX, maxLeft));
  const top = Math.max(margin, Math.min(event.clientY - active.offsetY, maxTop));
  Object.assign(active.dialog.style, { left: `${left}px`, top: `${top}px` });
  state.newProjectDialogPosition = { left, top };
  event.preventDefault();
});

const stopNewProjectDialogDrag = (event) => {
  const active = state.newProjectDialogDrag;
  if (!active || event.pointerId !== active.pointerId) {
    return;
  }
  state.newProjectDialogDrag = null;
  delete active.titleBar.dataset.windowDragging;
  if (active.titleBar.hasPointerCapture?.(event.pointerId)) {
    active.titleBar.releasePointerCapture(event.pointerId);
  }
};

for (const eventName of ["pointerup", "pointercancel", "lostpointercapture"]) {
  root.addEventListener(eventName, stopNewProjectDialogDrag);
}
              ...
              <div class="project-table-body">
                ${projects.length === 0 ? `<div class="empty-row">暂无历史项目。</div>` : projects.map((item) => `
                  <div class="project-row"
                    ...
                    <span class="project-actions-cell"><span class="project-open-cell">打开</span>
                      <button type="button" data-action="remove-recent-project"
                              data-product-id="${escapeAttr(product.productId)}"
                              data-project-path="${escapeAttr(item.path)}"
                              title="仅从最近项目中移除，不删除硬盘文件"
                              aria-label="移除 ${escapeAttr(item.displayName || getFileName(item.path))}"
                              ${state.pendingCount ? "disabled" : ""}>移除</button>
                    </span>
                  </div>
                `).join("")}
              </div>
...
function renderNewProjectDialog() {
  const product = getSelectedProductState();
  const position = state.newProjectDialogPosition;
  const positionStyle = position
    ? ` style="position:fixed;left:${escapeAttr(position.left)}px;top:${escapeAttr(position.top)}px;margin:0"`
    : "";
  return `
...
function getProgressTitle(label) {
  ...
  if (text === "Project.Save") return "保存项目";
...
function getProgressDetail(label) {
  const text = String(label ?? "");
  if (text === "Project.Save") return "正在保存项目数据和资源";
