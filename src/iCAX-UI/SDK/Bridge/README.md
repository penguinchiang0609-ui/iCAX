...
宿主 bridge 的必需能力是 application/product/scene channel 注册、`postSDOFrame`、`subscribeSDOFrames` 和直接资源访问 `requestResource`。`openFileDialog(options)` 是可选 UI 容器能力，CEF 容器会提供；没有该方法时，产品页面仍可以让用户手动输入路径。
