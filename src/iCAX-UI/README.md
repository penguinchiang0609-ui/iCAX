...
当前默认前端是 H5/CEF：`UIContainer + CefUIContainer + AppShell + AppProxy + ProductProxy + ProjectProxy + UI + SDK`。但底层 `UIContainer` 契约必须保持技术无关，WPF/QT 等前端只要实现 `IUIContainer` 并通过 `IFrontendBridge` 交互，就可以替换 H5/CEF。
