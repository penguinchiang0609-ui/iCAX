...
Mail、PDO、Bridge 是 `SDK` 内部通信能力，Viewport 是 H5 前端内置三维显示能力，UI 是公共界面能力。前端不拥有 repository、resource pool、ECS 等业务数据；这些数据仍由 Engine 管理，前端只持有状态快照、通道和视图状态。
...
- `SDK/`：给 `AppShell` 和产品 `webpage` 模块使用的统一导出门面，内部包含 Bridge、Mailbox、PDO、Viewport 和 H5 runtime 依赖。
