...
`iCAX-UI` 是 iCAX 的 H5 前端框架目录。它不是某个具体产品的页面，也不拥有 Engine。它提供 `UIContainer + CefUIContainer + AppShell + AppProxy + ProductProxy + ProjectProxy + UI + SDK` 这一套可替换的桌面 H5 前端运行框架。
...
AppShell
  -> SDK
    ...
    -> UI
    -> window.icax
      -> CefUIContainer / QtUIContainer / WpfUIContainer
        -> IFrontendBridge
