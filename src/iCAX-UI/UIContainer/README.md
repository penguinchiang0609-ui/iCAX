...
- `IUIContainer`：CEF、Qt、WPF、Headless 等 UI 容器共同实现的接口。
...
默认没有配置文件时，`Application.exe` 使用 WPF 容器。需要无窗口验收时，可显式配置 `type=headless`；它不创建窗口，只模拟前端初始化流程：获取 application channel，发送 `App.GetState`，等待后端响应。
