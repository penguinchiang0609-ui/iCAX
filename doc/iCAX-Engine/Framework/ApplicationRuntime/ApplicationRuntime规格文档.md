# ApplicationRuntime 规格文档

## 1. 定位

`ApplicationRuntime` 是应用级后台运行时，是前端启动后最先连接的稳定入口。
...
```cpp
iCAX::Application::ApplicationRuntimeConfig config;
...
`ApplicationRuntimeConfig` 包含：

- `strApplicationSettingsPath`：应用级配置文件。
- `Descriptor`：应用描述。
- `Paths`：应用路径。
- `Products`：运行时支持的产品定义列表。
- `StartupProductID`：运行时启动时自动启动的产品，可为空。
- `nFrameIntervalMilliseconds`：应用运行时线程帧间隔，只用于应用级消息轮询。

## 3. 生命周期

`Start` 启动后台工作线程，并等待运行时进入 `Running` 或 `Faulted`。
...
同一 `productId` 的 `StartProduct` 和 `StopProduct` 严格串行。产品停止期间，新的 `StartProduct` 会等待旧 `ProductRuntime` 完成停止并从运行时表中移除后，再创建新的运行时实例。
...
auto bytes = iCAX::Application::EncodeApplicationRuntimePayload(
...
`App.OpenProjectFile` 只接收文件路径和可选显示名称，不接收产品 ID。ApplicationRuntime 只用文件头 magic 识别产品。magic 不能为空，且必须在 ApplicationRuntime 的产品定义列表中全局唯一。扩展名只用于文件选择器和展示，不参与产品识别。若 magic 未命中则返回 `NotFound`；若出现多个产品命中同一个文件，则视为产品定义错误。
...
`ApplicationRuntime` 不应被 `Product`、`Project`、`Database`、`Behaviour`、`Resources` 或业务模块反向依赖。
