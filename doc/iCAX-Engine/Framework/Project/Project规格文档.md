...
`Project` 不直接拥有 Repository、Universe、ResourceLibrary、PDOHub、FacadeChannel 或工作线程。这些运行期对象全部归属 `Scene`。Project 启动后会创建一个主 Scene；导入预览、局部编辑、仿真或临时转换可以由已有 Scene 打开子 Scene。

## 2. 上下文边界

- `IProjectContext`：只提供 ProjectID、ProjectName、ProjectPath 和 ProjectSetting。
- `ISceneContext`：提供 SceneID、Scene Facade、Repository、ResourceLibrary、PDOHub、ServiceProvider 和 Scene 运行现场。
...
`CProject` 创建时需要注入应用、产品、服务、元数据、行为、资源加载器和 Facade 注册表等依赖。Project 使用这些依赖创建主 Scene：
...
info.pFacadeChannelRegistry = facadeChannelRegistry;
...
Project 中保留的 `MainSceneDatabase()`、`MainSceneResources()`、`MainScenePDOHub()`、`GetMainSceneFrontendFacadeEndpoint()` 等主 Scene 便利方法只转发到主 Scene，不能理解为 Project 自己拥有这些运行时资源。
...
- 一个 Scene Facade channel，用于前后端低频通信。
- 一个可选 PDOHub，用于高频共享内存同步。
- 一个后台工作线程，驱动 `PreSwapPDO`、Facade frame 处理、`Tick` 和 `PostSwapPDO`。
...
scene.SetFrameHandler([](
    ...
    const iCAX::Facade::CFacadeEndpoint& backendOffice) {
...
Scene 前端 Facade endpoint 可低成本获取：

```cpp
auto frontendOffice = scene.GetFrontendFacadeEndpoint();
```

`Close()` 会停止 Scene 线程、关闭 Scene Facade channel，并释放 Universe、Repository、ResourceLibrary 和 PDOHub。
...
`IProjectRuntime` 是 ProductRuntime 看到的项目运行句柄。它提供项目身份、状态、主 Scene 前端 Facade endpoint、启动、停止和关闭能力，但不要求上层知道项目内部是不是 `CProject`。
...
catalogInfo.pFacadeChannelRegistry = facadeChannelRegistry;
...
- Scene 之间的 Repository、ResourceLibrary、Universe、Facade channel、PDOHub 和后台线程互相隔离。
- Project 必须显式注入 MetaRegistry、BehaviourRegistry、ResourceLoaderRegistry 和 FacadeChannelRegistry，不读取全局注册表。
