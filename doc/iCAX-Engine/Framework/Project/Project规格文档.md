...
`Project` 不直接拥有 Repository、Universe、ResourceLibrary、PDOHub、SDOChannel 或工作线程。这些运行期对象全部归属 `Scene`。Project 启动后会创建一个主 Scene；导入预览、局部编辑、仿真或临时转换可以由已有 Scene 打开子 Scene。

## 2. 上下文边界

- `IProjectContext`：只提供 ProjectID、ProjectName、ProjectPath 和 ProjectSetting。
- `ISceneContext`：提供 SceneID、Scene SDO、Repository、ResourceLibrary、PDOHub、ServiceProvider 和 Scene 运行现场。

Behaviour、SDO 和 RenderService 需要同时拿到 ProjectContext 与 SceneContext。ProjectContext 不能替代 SceneContext，SceneContext 也不能遮盖 ProjectContext。

## 3. Project

`CProject` 创建时需要注入应用、产品、服务、元数据、行为、资源加载器和 SDO 注册表等依赖。Project 使用这些依赖创建主 Scene：
...
info.pSDOChannelRegistry = sdoChannelRegistry;
...
Project 中保留的 `MainSceneDatabase()`、`MainSceneResources()`、`MainScenePDOHub()`、`GetMainSceneFrontendSDOEndpoint()` 等主 Scene 便利方法只转发到主 Scene，不能理解为 Project 自己拥有这些运行时资源。
...
- 一个 Scene SDO channel，用于前后端低频通信。
- 一个可选 PDOHub，用于高频共享内存同步。
- 一个后台工作线程，驱动 `PreSwapPDO`、SDO frame 处理、`Tick` 和 `PostSwapPDO`。
...
scene.SetFrameHandler([](
    ...
    const iCAX::SDO::CSDOEndpoint& backendOffice) {
    auto frames = backendEndpoint.Receive();
...
Scene 前端 SDO endpoint 可低成本获取：

```cpp
auto frontendOffice = scene.GetFrontendSDOEndpoint();
```

`Close()` 会停止 Scene 线程、关闭 Scene SDO channel，并释放 Universe、Repository、ResourceLibrary 和 PDOHub。
...
`IProjectRuntime` 是 ProductRuntime 看到的项目运行句柄。它提供项目身份、状态、主 Scene 前端 SDO endpoint、启动、停止和关闭能力，但不要求上层知道项目内部是不是 `CProject`。

当前实现是 `CProjectRuntime`，它包装进程内 `CProject`，实际工作线程由主 Scene 持有。若未来需要每个 Project 独立内存空间，应增加新的进程级实现，而不是让 ProductRuntime 直接依赖 `CProject` 内部对象。
...
catalogInfo.pSDOChannelRegistry = sdoChannelRegistry;
...
- Scene 之间的 Repository、ResourceLibrary、Universe、SDO channel、PDOHub 和后台线程互相隔离。
- Project 必须显式注入 MetaRegistry、BehaviourRegistry、ResourceLoaderRegistry 和 SDOChannelRegistry，不读取全局注册表。
