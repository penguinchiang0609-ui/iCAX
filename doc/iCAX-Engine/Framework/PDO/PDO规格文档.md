...
Arena 通常由 Scene 通过创建参数中的 `PDOHubCreateInfo` 创建和持有。SDO 可以用于通知前端：“某个 Scene 的 PDO Arena 名称是什么，以及某个 PDOID 对应的 slot 何时创建或释放”。Scene 关闭后会释放自己的 PDOHub，前端宿主应停止访问对应 Arena。
