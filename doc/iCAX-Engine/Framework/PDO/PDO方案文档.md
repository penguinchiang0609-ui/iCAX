...
`CPDOHub` 使用 mutex 保护 slot map、动态分配和释放。释放 slot 后，旧 `IPDOSlot&` 引用立即失效；业务层必须通过生命周期和 SDO 事件保证读写方不继续访问已释放 slot。
...
- 在 SDO 层补充项目关闭通知，提醒前端宿主停止访问对应 Arena。
