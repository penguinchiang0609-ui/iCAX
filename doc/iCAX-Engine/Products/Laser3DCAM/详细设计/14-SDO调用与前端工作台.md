...
产品前端通过 SceneProxy 调用 Laser3DCAM 对外 SDO。SDO 表达低频调用、结果和事件，PDO 承载高频显示和交互状态。

## 2. 方法身份

每个方法固定使用 `SDOName.MethodName`：
...
不再使用统一 `Cam` 主命令，也不提供 `Cam.Machine.Import` 三段名称。SDO 名称表达稳定业务接口，方法名称表达行为。
...
SDO 负责参数校验、上下文选择、事务边界和领域编排。可复用算法继续放在 Service，持久化数据继续放在 Component/Resource。
...
前端不直接构造 SDO，不直接计算 methodCode，也不访问后端 Entity/Component。
...
工件、意图刀路、工艺、ProcessPath、机床实例、作业和安全检查结果属于后端主数据，前端只能通过 SDO、PDO 和 Resource 访问。
...
- 不存在实例级 SDO 注册；
- 不接受三段方法名；
- Promise 可由结果 SDO 正确完成或拒绝；
- 跨 SDO 范围调用返回失败结果，不使 Scene 线程故障；
