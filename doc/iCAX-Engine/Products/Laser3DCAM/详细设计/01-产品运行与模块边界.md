...
- 不直接拥有 Repository、ResourcePool、PDOHub、MailChannel 或 Universe。

Scene：

- 持有 SceneContext。
- 拥有独立 Repository、ResourcePool、PDOHub、MailChannel 和 Universe。
- 拥有自己的 tick 调度。
...
- 项目创建失败：ApplicationHost 返回失败，不创建半初始化 Project 或 Scene。
...
- Scene 拥有独立 Repository、Resources、PDOHub、MailChannel。
