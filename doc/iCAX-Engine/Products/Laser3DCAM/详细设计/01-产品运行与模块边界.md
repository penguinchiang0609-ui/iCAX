...
- 暴露 Application 级 SDO channel。

ProductRuntime：

- 持有 ProductContext。
- 加载产品插件。
- 注册产品级 ComponentMeta、Behaviour、Service、SDO。
...
- 不直接拥有 Repository、ResourcePool、PDOHub、SDOChannel 或 Universe。

Scene：

- 持有 SceneContext。
- 拥有独立 Repository、ResourcePool、PDOHub、SDOChannel 和 Universe。
- 拥有自己的 tick 调度。
...
- 从 View 和资源池读取场景与渲染数据；相机和拖拽预览在本地处理。
- 不直接持有项目主数据。
...
- 前端与后端通过 SDO 异步通信。
- 项目命令通过 SDO 交换，场景投影通过 View 资源交换；Collider PDO 仅用于可丢帧的碰撞调试结果。
...
- Scene 拥有独立 Repository、Resources、PDOHub、SDOChannel。
