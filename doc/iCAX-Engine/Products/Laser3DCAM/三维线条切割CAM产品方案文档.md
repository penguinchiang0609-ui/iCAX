...
规格文档回答“产品要做什么”，本文回答“在现有 Application、Product、Project、Database、Resources、SDO、PDO、Service、Behaviour、UI SDK 体系中如何落地”。

本文只描述三维线条切割 CAM 产品方案。通用框架能力仍由 `iCAX-Engine`、`iCAX-UI` 和通用插件目录维护，产品专属逻辑不得写入 framework。

## 2. 设计原则

- Project 是项目文件和项目参数边界；主 Scene 是数据运行边界，项目主数据进入主 Scene.Database。
...
- Project 负责项目级参数和 Scene 管理；Scene 负责数据库、资源、PDO、SDO、universe 和 tick。
...
sequenceDiagram
    ...
    participant SceneProxy as SceneProxy
    ...
    Host->>Host: 创建 Project、MainScene、ProjectContext 和 SceneContext
    Host-->>ProjectProxy: 返回 project state + main scene state
    ProjectProxy->>SceneProxy: registerSceneChannel(projectId, mainSceneId)
    SceneProxy->>SceneProxy: 进入 laser-3d-cam 页面
```
...
- 项目坐标、单位和默认误差。
- 工件集合由多个 Workpiece Entity 表达，`Root.ActiveWorkpieceID` 只保存当前激活工件引用。
- 当前切割头引用。
- 图层。
- 已确认刀路。
- 刀路工艺。
...
- `RawCadResource`：目标工件 STEP/STP、IGS/IGES 原始文件资源。
- `BRepModelResource`：导入后的 `GeometryData::BRepModel` 中立几何资源。
...
- 渲染缓存可以保存，也可以重新生成，但不能成为 CAM 权威几何。
...
SDO 保存命令交互。payload 使用 JSON 字符串表达，返回值也使用 JSON 字符串表达。
...
目标工件只支持 STEP/STP、IGS/IGES。STL 不作为目标工件导入格式。一个项目可以导入多个目标工件，每个工件都是独立 Workpiece Entity；当前工件由 `CLaserCamRootComponent.ActiveWorkpieceID` 指定，刀路通过 `CCAMPathComponent.WorkpieceEntityID` 归属到具体工件。
...
`CompositeCurve3` 使用 `GeometryData` 中立几何数据表达，包含：
...
采样点使用 `SegmentIndex + SegmentU` 绑定 `PathCurveResource.Curve.Segments[]` 中的曲线段和该段原生参数。`BeamDirection` 使用世界坐标。采样点位置不持久化在 PoseFieldResource 中，而是由曲线资源按 `SegmentIndex + SegmentU` 求值获得。姿态场不保存 TCP 字段、完整机械姿态或关节值。
...
服务从 ProjectContext 获取 Repository、Resources、PDOHub、SDOChannel、ServiceProvider 和 Universe。
...
- 保存原始 CAD 文件为内嵌资源。
- 生成 `GeometryData::BRepModel` 中立 BRep 资源。
- 生成可拾取拓扑索引资源。
- 可选生成或标记 render 显示数据，并生成碰撞资源。
- 将导入几何转换到项目世界坐标。
- 返回资源 ID 和版本；不直接写 Repository。
...
- 控制默认最大几何偏差 `0.001 mm`。
- 输出 GeneratedToolpath，由 SDO 写入 PathCurveResource、PathEntity 和 Block.Children。
- 不让刀路主身份依赖 CAD 拓扑。
...
Cam.ImportTool
Cam.ImportModel
Cam.SetActiveWorkpiece
Cam.RecognizeFeatures
Cam.PickObject
Cam.GenerateToolpath
Cam.UpdateToolpathProcess
Cam.UpdateLayer
Cam.PlanOrder
Cam.GeneratePoseField
Cam.GenerateMotionPlan
Cam.RunSafetyCheck
Cam.StartSimulation
Cam.PauseSimulation
Cam.ResumeSimulation
Cam.StopSimulation
Cam.ResetSimulation
```
...
sequenceDiagram
    participant UI as ProjectProxy
    participant Cmd as LaserCam SDO
    participant Cad as CadImportService
    participant Repo as Database
    participant Res as Resources
    participant Render as CamRenderSyncService

    UI->>Cmd: Cam.ImportModel(path)
    Cmd->>Cad: Import STEP/IGES
    Cad->>Res: 内嵌 RawCadResource
    Cad->>Res: 生成 BRepModelResource
    Cad->>Res: 生成 TopologyResource
    Cad->>Render: 可选生成或标记渲染显示数据
    Cad->>Res: 生成碰撞资源
    Cad-->>Cmd: 返回资源 ID 和版本
    Cmd->>Repo: 追加 WorkpieceEntity 并设为 ActiveWorkpieceID
    Cmd->>Render: 请求刷新工件显示 PDO
    Cmd-->>UI: 返回工件列表、当前工件和状态
```
...
sequenceDiagram
    ...
    UI->>Cmd: Cam.RecognizeFeatures()
    Cmd->>Rec: BRep + FeatureDefinition[] 识别孔和坡口孔
    Rec-->>Cmd: RecognizedFeature[]
    Cmd-->>UI: 返回参数化特征列表
    UI->>Cmd: Cam.PickObject(edge/loop)
    Cmd->>Pick: 更新当前选择
    UI->>Cmd: Cam.GenerateToolpath(recognizedFeature)
...
sequenceDiagram
    ...
    UI->>Cmd: Cam.GenerateMotionPlan(toolpathIDs)
    Cmd->>Pose: 生成姿态场
...
碰撞检测作为 Scene 级业务服务运行。服务实例可以来自应用级 `ServiceProvider`，但碰撞场景数据必须按 Scene 隔离。
