...
product.Modules.CommandModules.push_back("bin/FlatLaserCamCommand.dll");
```
...
当前框架已有 `CommandRegistry`、`CommandDispatcher` 和 `CommandRegistrationCatalog`。产品命令模块通过 `ICAX_REGISTER_COMMAND` 自动注册，ProductRuntime 加载产品 DLL 后按模块路径回放注册。
