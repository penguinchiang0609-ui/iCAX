...
Mail 通信不处理业务逻辑，不解析 Payload。需要按消息类型调用函数时，上层可用 `MailHeader::nTypeCode` 对接 `ProcedureCall`。
...
- `Services` 可把通用 EndA/EndB 映射成具体产品中的两个运行角色。
...
- `nTypeCode`：邮件类型码，常见用法是保存 ProcedureCall 的 `PCID`。
...
### 9.3 与 ProcedureCall 配合

```cpp
#include <ProcedureCall/IPCRegistry.h>

auto office = channel.GetPostOffice(kMailEndB);
auto mails = office.Receive();
...
for (auto& mail : mails)
{
    ...
    if (fn != nullptr)
    {
        fn(&context, &mail.Payload, nullptr);
    }
...
这里的变量名称属于服务层或产品层，`CMailChannel` 本身只知道 EndA/EndB。
