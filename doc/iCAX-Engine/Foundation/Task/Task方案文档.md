...
Task 工程位于 `src/icax-engine/foundation/Task/`，属于 Foundation 层基础能力。
...
`EventLoopTaskScheduler` 放在头文件中，供 Engine/UI 单线程 event loop 和单元测试直接使用。
...
### 2.6 Coroutine.h / Coroutine.cpp

负责 C++20 Coroutine 适配：

- move-only `CCoroutine<TResult>` frame 与 typed promise。
- `CCoroutineRuntime` 和 `CCoroutineHandle<TResult>`。
- next-frame、时间、条件与 Task awaitable。
- 基于 runtime 身份和 coroutine ID 的迟到唤醒校验。

Coroutine runtime 不持有具体 backend/frontend owner。上层把自己的 `EventLoopTaskScheduler` 注入 runtime，保存启动后返回的 handle，并在 owner 禁用、启用、销毁时显式暂停、恢复或取消。Task continuation 只捕获 runtime 弱引用和数值 ID，不捕获原始 coroutine handle，从而避免 Task 完成晚于 owner 销毁时访问悬空 frame。
...
1. 使用调用方传入的 scheduler 创建 `TaskCompletionSource`；未传入时使用 `DefaultScheduler()`。
...
未显式传入 scheduler 时，`StartNew` 使用线程池形式的 `DefaultScheduler()`；只有调用方主动传入 scheduler 时才改变任务执行位置。
...
1. 未显式传 scheduler 时继承前序 Task 的 scheduler；显式传入时使用调用方指定值。
2. 使用选定 scheduler 创建 continuation 的 `TaskCompletionSource`，使返回 Task 继续继承相同调度语义。
3. 检查前序任务是否有效。
4. 根据取消令牌和 `LazyCancellation` 决定是否立即取消。
5. 将 continuation 注册到前序任务。
6. 前序任务进入终态后，判断 `TaskContinuationOptions` 条件是否匹配。
...
- 否则使用显式传入的 scheduler；未显式传入时使用前序 Task 绑定的 scheduler。
...
- Coroutine 只支持显式 `Await(Task<T>)`，不为 Task 增加隐式 `operator co_await`。
- 不支持真实 UI 同步上下文。
...
```text
src/tests/icax-engine/foundation/Task/TaskTest/
```
...
5. 持续验证 Coroutine owner scope 在 Engine/UI 生命周期中的销毁与迟到唤醒安全性。
