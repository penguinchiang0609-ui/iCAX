...
- Project 是数据和运行隔离边界。
- Service、ComponentMeta、Behaviour 定义和 ResourceLoader 在 Application 级共享。
- MailChannelService 在 Application 级共享，但 channel 按 mail id 显式创建和删除。
- Repository、资源对象、Universe 实例、项目 mail channel 和项目线程按 Project 隔离。
...
ApplicationHost
  ...
  -> ProductRuntime.Start()
       ...
       replay module ComponentMeta registrations
       replay module Behaviour registrations
       replay module ResourceLoader registrations
...
Behaviour 定义在 Application 级注册，Project 创建自己的 Universe 实例。Universe 不持有数据；Project 持有 UniverseContext，把当前 Repository、应用设置和时间信息传给 Behaviour。

因此 Behaviour 定义可以复用，但行为执行上下文属于当前 Project。
...
ResourceLoader 定义按 Application 注册：
...
ProductRuntime 加载产品模块后，按模块路径把注册动作回放到 ApplicationHost 持有的应用级容器或注册表。
