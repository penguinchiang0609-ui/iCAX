...
`PDORenderService` 不负责渲染屏幕。它只负责把后端渲染数据同步到 Scene PDO，并通过 Scene SDO 告知前端 slot 的分配、释放和移动。
...
`PDORenderService` 通过 Scene 后端 SDO endpoint 发送 Event，前端从对应 endpoint 接收。payload 是 UTF-8 JSON 文本。所有 64 位 ID 都以字符串形式写入，避免 JS `Number` 精度问题。
