...
- 普通交互走 SDO，返回值以 Promise 形式表达。
- 高频数据走 PDO，SDO 只传递控制命令和状态通知。
...
`UIContainer` 不实现任何具体产品逻辑，不启动/停止 `ApplicationRuntime`，不缓存 SDO endpoint。

`CefUIContainer` 负责 CEF runtime、浏览器窗口、`window.icax` 注入和 Engine SDO 推送。PDO shared memory 到 JS `ArrayBuffer`、文件对话框、窗口标题、拖拽文件属于 `CefUIContainer` 的后续宿主能力。
...
底层会生成 command route，序列化 payload，发送 SDO，并等待 Engine 通过 `callId` 返回 response。
...
Application、Product、Scene 有自己的 SDO 入口。Project 是项目容器，不拥有 SDO。前端先和 application 对话，再按需进入 product，打开 project 后默认使用主 Scene 对话。
...
backend observer/event
  -> backend SDO endpoint Send(callId = 0)
  -> FrontendBridge
  -> UIContainer
  -> window.icax event callback
  -> SDO subscribe/subscribeAll
  -> UI update
```
