...
设计上必须支持切换为 WPF/QT 等其他前端技术。切换条件不是改 backend，而是对应前端容器实现 `IUIContainer`，通过 `IFrontendBridge` 发送/接收 SDO，并按需暴露 PDO 读取能力。
...
- `UI/`：公共 UI 基础模块规格和方案。
- `SDK/`：前端统一导出门面规格和方案，内部包含 AppShell、Bridge、SDO、PDO 文档。
