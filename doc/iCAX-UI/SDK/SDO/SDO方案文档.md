...
代码位于 `src/iCAX-UI/SDK/SDO/`：

- `sdoClient.mjs`：双向调用、汇报、响应、事件、超时和生命周期；
- `sdoMethod.mjs`：与 C++ 一致的稳定方法编码；
...
```text
前端调用后端：SDOClient.invoke() -> Promise
后端调用前端：SDOClient.expose() 处理 Request -> Response
```
...
`postSDOFrame()` 只表示 frame 已投递，不表示业务完成。业务完成只由后续 Response 表达，因此 Bridge 不同步阻塞等待业务线程。

## 4. 前端方法分发

`expose(channelId, sdoMethod, handler)` 按 channel 和方法码登记前端实现。收到 Request 后：
