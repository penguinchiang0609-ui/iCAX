...
`SDO` 是前端 SDK 的调用边界。页面通过 Proxy 或 `SDOClient` 调用后端 SDO，也可以用 `expose()` 向后端提供前端 SDO。公开 API 只表达方法、参数、汇报和结果，不要求页面理解内部传输实现。

## 2. 方法模型

公开名称固定为 `SDOName.MethodName`，只允许两个 PascalCase 标识符段。例如 `Machine.Jog` 合法，`Cam.Machine.Import` 非法。实例信息必须放在参数中：

```js
sdo.invoke(channelId, "Machine.Jog", { machineId, axis, delta });
```
...
`SDOClient.invoke()` 立即返回 Promise，不同步等待后端。返回的 Promise 额外提供：
...
```js
const unexpose = sdo.expose(channelId, "Workbench.GetSelection", async (payload, context) => {
