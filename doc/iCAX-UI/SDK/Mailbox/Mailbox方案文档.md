...
`MailboxClient.invoke()` 根据 `FacadeName.MethodName` 创建 mail，投递给 bridge，并在 response `originId` 匹配时 resolve。

超时或 backend 返回错误时 reject。

## 3. Facade 方法编码

`facadeMethod.mjs` 使用和 C++ 一致的 FNV-1a 32 位算法：
...
同一个 mailbox 第一次调用时建立订阅，之后复用订阅。

这种方式避免每次调用都重复注册 host callback。
...
- `makeFacadeMethodCode("App", "GetState")`
- `makeFacadeMethodCodeFromName(AppFacade.getState)`
