...
具体 UI 容器负责把这些调用转成 `IFrontendBridge` 调用。H5 前端 Promise 由 `iCAX-UI/SDK/SDO` 管理；原生后端使用 `Task<CInvocationResult>`。无论调用方向如何，Bridge 都只异步投递 frame，不在收帧线程同步等待业务完成。WPF/QT 前端也应使用 Task/async 包装同一 Request/Report/Response 语义。
