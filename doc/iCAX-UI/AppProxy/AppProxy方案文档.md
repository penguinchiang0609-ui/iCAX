...
AppProxy.create(bridge)
  ...
  -> new SDOClient(bridge)
  -> new AppProxy(SDOClient, applicationChannelId)
```
...
`registerProductChannel` 只是让 `FrontendBridge` 缓存 product SDO endpoint，不代表业务启动。业务启动由 `App.StartProduct` 决定。
